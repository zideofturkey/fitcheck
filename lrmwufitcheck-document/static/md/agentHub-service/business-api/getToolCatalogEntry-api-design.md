...
The event will be emitted to the `toolcatalogentry-retrived` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
```js
runMScript(() => ({id:this.sys_toolCatalogId}), {"path":"services[5].businessLogic[6].whereClause.fullWhereClause"})
```
