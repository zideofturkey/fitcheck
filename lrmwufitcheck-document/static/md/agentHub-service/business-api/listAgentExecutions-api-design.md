...
The event will be emitted to the `agentexecutions-listed` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
- Single value: `?agentType=<value>` (case-insensitive)
- Multiple values: `?agentType=<value1>&agentType=<value2>`
- Null check: `?agentType=null`
...
- Single value: `?source=<value>` (case-insensitive)
- Multiple values: `?source=<value1>&source=<value2>`
- Null check: `?source=null`
...
The `listAgentExecutions` api has 5 filter parameters available for filtering list results. See the [Filter Parameters](#filter-parameters) section above for detailed usage examples.
