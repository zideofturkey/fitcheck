...
The event will be emitted to the `agentexecution-retrived` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
```js
runMScript(() => ({id:this.sys_agentExecutionId}), {"path":"services[5].businessLogic[8].whereClause.fullWhereClause"})
```
