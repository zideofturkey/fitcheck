...
The event will be emitted to the `toolcatalog-listed` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
- Single value: `?serviceName=<value>` (matches any string containing the value, case-insensitive)
- Multiple values: `?serviceName=<value1>&serviceName=<value2>` (matches records containing any of the values)
- Null check: `?serviceName=null`
...
The `listToolCatalog` api has 1 filter parameter available for filtering list results. See the [Filter Parameters](#filter-parameters) section above for detailed usage examples.
