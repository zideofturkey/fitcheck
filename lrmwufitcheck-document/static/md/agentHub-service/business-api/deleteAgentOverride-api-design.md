...
The event will be emitted to the `agentoverride-deleted` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
```js
runMScript(() => ({id:this.sys_agentOverrideId}), {"path":"services[5].businessLogic[4].whereClause.fullWhereClause"})
```
