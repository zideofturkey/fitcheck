...
### [8] Action : callParseMealWithAI
**Action Type**: `FunctionCallAction`

Invokes LIB.parseMealWithAI to parse the Turkish meal description into structured nutrition data

```js
class Api {
  async callParseMealWithAI() {
    try {
      return runMScript(
        () =>
          LIB.parseMealWithAI(
      ...
      console.error("Error in FunctionCallAction callParseMealWithAI:", err);
      throw err;
    }
  }
}
...
### [9] Action : updateSessionWithParseResult
...
### [11] Action : loopCreateCandidateLines
