...
# Business API Design Specification - `Consume Invitelink`

A Business API is a set of logical actions centered around a main data object. These actions can range from simple CRUD operations to complex workflows that implement intricate business logic.

While the term “API” traditionally refers to an interface that allows software systems to interact, in Mindbricks a Business API represents a broader concept. It encapsulates a business workflow around a data object, going beyond basic CRUD operations to include rich, internally coordinated actions that can be fully designed and customized.

This document provides an in-depth explanation of the architectural design of the `consumeInviteLink` Business API. It is intended to guide backend architects and developers in maintaining the current design. Additionally, frontend developers and frontend AI agents can use this document to understand how to properly consume this API on the client side.

## Main Data Object and CRUD Operation
The `consumeInviteLink` Business API is designed to handle a `update` operation on the `InviteLink` data object. This operation is performed under the specified conditions and may include additional, coordinated actions as part of the workflow.
...
The event will be emitted to the `invitelink-consumed` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
The `consumeInviteLink` Business API includes a REST controller that can be triggered via the following route:
...
REST controllers also expose the Business API as a tool in the MCP, making it accessible to AI agents. This `consumeInviteLink` Business API will be registered as a tool on the MCP server within the service binding.

## API Parameters

The `consumeInviteLink` Business API has 4 parameters that must be sent from the controller. Note that all parameters, except session and Redis parameters, should be provided by the client.
...
|  |                       |                                        |                                                                      |                             |                                | 
| `registeredUserId`                             | `ID` | `Yes` | `-` | `body` | `registeredUserId` |
| **Description:**                                | The auth user id created from this invite |                              |                                                                      |                             |                                |
|  |                       |                                        |                                                                      |                             |                                | 
| `relatedEmail`                             | `String` | `No` | `-` | `body` | `relatedEmail` |
...
The **authentication and authorization configuration** defines the core access rules for the `consumeInviteLink` Business API. These checks are applied **after parameter validation** and before executing the main business logic.
...
```js
runMScript(() => ({ id: this.inviteLinkId }), {"path":"services[1].businessLogic[5].whereClause.fullWhereClause"})
```
...
```js
{
  registeredUserId: runMScript(() => (this.registeredUserId), {"path":"services[1].businessLogic[5].dataClauseItems[0].value"}),
  inviteState: 'consumed',
  lastUsedAt: runMScript(() => (LIB.now()), {"path":"services[1].businessLogic[5].dataClauseItems[2].value"}),
}
```
...
### [8] Action : checkInviteLinkExists
...
### [12] Action : createAuditOnConsume
...
The `consumeInviteLink` api has got 3 regular client parameters  
