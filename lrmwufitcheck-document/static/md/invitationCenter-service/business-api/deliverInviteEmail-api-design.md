...
# Business API Design Specification - `Deliver Inviteemail`

A Business API is a set of logical actions centered around a main data object. These actions can range from simple CRUD operations to complex workflows that implement intricate business logic.

While the term “API” traditionally refers to an interface that allows software systems to interact, in Mindbricks a Business API represents a broader concept. It encapsulates a business workflow around a data object, going beyond basic CRUD operations to include rich, internally coordinated actions that can be fully designed and customized.

This document provides an in-depth explanation of the architectural design of the `deliverInviteEmail` Business API. It is intended to guide backend architects and developers in maintaining the current design. Additionally, frontend developers and frontend AI agents can use this document to understand how to properly consume this API on the client side.

## Main Data Object and CRUD Operation
The `deliverInviteEmail` Business API is designed to handle a `update` operation on the `InviteLink` data object. This operation is performed under the specified conditions and may include additional, coordinated actions as part of the workflow.
...
The `deliverInviteEmail` Business API includes a REST controller that can be triggered via the following route:
...
REST controllers also expose the Business API as a tool in the MCP, making it accessible to AI agents. This `deliverInviteEmail` Business API will be registered as a tool on the MCP server within the service binding.

## API Parameters

The `deliverInviteEmail` Business API has 2 parameters that must be sent from the controller. Note that all parameters, except session and Redis parameters, should be provided by the client.
...
The **authentication and authorization configuration** defines the core access rules for the `deliverInviteEmail` Business API. These checks are applied **after parameter validation** and before executing the main business logic.
...
```js
runMScript(() => ({ id: this.inviteLinkId, inviteState: 'active' }), {"path":"services[1].businessLogic[3].whereClause.fullWhereClause"})
```
...
```js
{
  deliveryRequestedAt: runMScript(() => (LIB.now()), {"path":"services[1].businessLogic[3].dataClauseItems[0].value"}),
}
```
...
### [8] Action : checkActiveWithEmail
**Action Type**: `ValidationAction`

Verify the invite link is active and has an invitedEmail before delivering
...
### [9] Action : fetchInviteLinkRecord
...
### [13] Action : publishDeliveryEvent
**Action Type**: `PublishEventAction`

Publish a Kafka event for the notification service to send the invite email
...
### [14] Action : createAuditOnDeliver
...
The `deliverInviteEmail` api has got 1 regular client parameter  
