...
# Business API Design Specification - `List Nutritiondays`

A Business API is a set of logical actions centered around a main data object. These actions can range from simple CRUD operations to complex workflows that implement intricate business logic.

While the term “API” traditionally refers to an interface that allows software systems to interact, in Mindbricks a Business API represents a broader concept. It encapsulates a business workflow around a data object, going beyond basic CRUD operations to include rich, internally coordinated actions that can be fully designed and customized.

This document provides an in-depth explanation of the architectural design of the `listNutritionDays` Business API. It is intended to guide backend architects and developers in maintaining the current design. Additionally, frontend developers and frontend AI agents can use this document to understand how to properly consume this API on the client side.

## Main Data Object and CRUD Operation
The `listNutritionDays` Business API is designed to handle a `list` operation on the `NutritionDay` data object. This operation is performed under the specified conditions and may include additional, coordinated actions as part of the workflow.
...
The `listNutritionDays` Business API includes a REST controller that can be triggered via the following route:
...
REST controllers also expose the Business API as a tool in the MCP, making it accessible to AI agents. This `listNutritionDays` Business API will be registered as a tool on the MCP server within the service binding.

## API Parameters

The `listNutritionDays` Business API has 3 parameters that must be sent from the controller. Note that all parameters, except session and Redis parameters, should be provided by the client.
...
The `listNutritionDays` api supports 1 optional filter parameter for filtering list results using URL query parameters. These parameters are only available for `list` type APIs.

#### `summaryDate` Filter

**Type:** `Date`  
**Description:** Filter by summaryDate  
...
- Special operators: `?summaryDate=$today`, `?summaryDate=$week`, `?summaryDate=$month`
- Local timezone: `?summaryDate=$ltoday`, `?summaryDate=$lweek`, `?summaryDate=$leq-2024-01-15`, `?summaryDate=$lin-2024-01-15&summaryDate=$lin-2024-01-20`
- Null check: `?summaryDate=null`
...
GET /v1/nutrition-days?summaryDate=2024-01-15

// Get records created on multiple dates (use multiple parameters)
GET /v1/nutrition-days?summaryDate=2024-01-15&summaryDate=2024-01-20

// Get records created today (server timezone)
GET /v1/nutrition-days?summaryDate=$today

// Get records created today (user's local timezone)
GET /v1/nutrition-days?summaryDate=$ltoday

// Get records created this week (server timezone)
GET /v1/nutrition-days?summaryDate=$week

// Get records created this week (user's local timezone)
GET /v1/nutrition-days?summaryDate=$lweek

// Get records created this month
GET /v1/nutrition-days?summaryDate=$month

// Get records created on a specific date (user's local timezone)
GET /v1/nutrition-days?summaryDate=$leq-2024-01-15

// Get records created on multiple dates (user's local timezone, use multiple parameters)
GET /v1/nutrition-days?summaryDate=$lin-2024-01-15&summaryDate=$lin-2024-01-20

// Get records without this field
GET /v1/nutrition-days?summaryDate=null
```
...
The **authentication and authorization configuration** defines the core access rules for the `listNutritionDays` Business API. These checks are applied **after parameter validation** and before executing the main business logic.
...
```js
runMScript(() => ({$and:[{ userId: this.session.userId },{userId:this.session?.userId}]}), {"path":"services[3].businessLogic[11].whereClause.fullWhereClause"})
```
...
The `listNutritionDays` api has got 2 regular client parameters  
...
The `listNutritionDays` api has 1 filter parameter available for filtering list results. See the [Filter Parameters](#filter-parameters) section above for detailed usage examples.

| Filter Parameter       | Type                   | Array Property | Description                   |
| ---------------------- | ---------------------- | -------------- | ----------------------------- |
| summaryDate  | Date  | No | Filter by summaryDate |
