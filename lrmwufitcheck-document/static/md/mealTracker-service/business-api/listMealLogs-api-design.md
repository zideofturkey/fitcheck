...
```js
runMScript(() => ({$and:[{ userId: this.session.userId },{userId:this.session?.userId}]}), {"path":"services[3].businessLogic[2].whereClause.fullWhereClause"})
```
