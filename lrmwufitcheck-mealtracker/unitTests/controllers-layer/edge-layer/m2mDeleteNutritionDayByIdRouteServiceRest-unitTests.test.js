...
describe("m2mDeleteNutritionDayByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteNutritionDayByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteNutritionDayByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteNutritionDayById-rest",
      {
        edgeFunctions: { m2mDeleteNutritionDayById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteNutritionDayByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteNutritionDayByIdRestController = (req, res, next) => {
      next(error);
    };
