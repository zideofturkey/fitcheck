...
describe("m2mUpdateNutritionDayByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateNutritionDayByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateNutritionDayByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateNutritionDayById-rest",
      {
        edgeFunctions: { m2mUpdateNutritionDayById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateNutritionDayByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateNutritionDayByIdRestController = (req, res, next) => {
      next(error);
    };
