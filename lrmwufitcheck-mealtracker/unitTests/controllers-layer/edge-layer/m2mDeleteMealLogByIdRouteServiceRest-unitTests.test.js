...
describe("m2mDeleteMealLogByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteMealLogByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteMealLogByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteMealLogById-rest",
      {
        edgeFunctions: { m2mDeleteMealLogById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteMealLogByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteMealLogByIdRestController = (req, res, next) => {
      next(error);
    };
