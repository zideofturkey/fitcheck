...
describe("m2mBulkCreateMealLogRestController", function () {
  let functionNameStub;
  let m2mBulkCreateMealLogRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateMealLogRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateMealLog-rest",
      {
        edgeFunctions: { m2mBulkCreateMealLog: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateMealLogRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateMealLogRestController = (req, res, next) => {
      next(error);
    };
