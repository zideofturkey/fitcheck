...
describe("m2mUpdateMealLogByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLogByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLogByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLogById-rest",
      {
        edgeFunctions: { m2mUpdateMealLogById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLogByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLogByIdRestController = (req, res, next) => {
      next(error);
    };
