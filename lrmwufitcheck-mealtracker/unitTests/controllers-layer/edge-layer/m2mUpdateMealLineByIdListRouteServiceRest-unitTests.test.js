...
describe("m2mUpdateMealLineByIdListRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLineByIdListRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLineByIdListRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLineByIdList-rest",
      {
        edgeFunctions: { m2mUpdateMealLineByIdList: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLineByIdListRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLineByIdListRestController = (req, res, next) => {
      next(error);
    };
