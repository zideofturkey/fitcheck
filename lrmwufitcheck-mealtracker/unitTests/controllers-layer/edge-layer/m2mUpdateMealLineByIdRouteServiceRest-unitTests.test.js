...
describe("m2mUpdateMealLineByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLineByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLineByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLineById-rest",
      {
        edgeFunctions: { m2mUpdateMealLineById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLineByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLineByIdRestController = (req, res, next) => {
      next(error);
    };
