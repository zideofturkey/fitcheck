...
describe("m2mCreateMealLogRestController", function () {
  let functionNameStub;
  let m2mCreateMealLogRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateMealLogRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateMealLog-rest",
      {
        edgeFunctions: { m2mCreateMealLog: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateMealLogRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateMealLogRestController = (req, res, next) => {
      next(error);
    };
