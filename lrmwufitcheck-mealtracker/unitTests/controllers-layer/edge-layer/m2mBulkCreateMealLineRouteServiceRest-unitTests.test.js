...
describe("m2mBulkCreateMealLineRestController", function () {
  let functionNameStub;
  let m2mBulkCreateMealLineRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateMealLineRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateMealLine-rest",
      {
        edgeFunctions: { m2mBulkCreateMealLine: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateMealLineRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateMealLineRestController = (req, res, next) => {
      next(error);
    };
