...
describe("m2mUpdateMealLogByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLogByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLogByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLogByQuery-rest",
      {
        edgeFunctions: { m2mUpdateMealLogByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLogByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLogByQueryRestController = (req, res, next) => {
      next(error);
    };
