...
describe("m2mUpdateMealLogByIdListRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLogByIdListRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLogByIdListRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLogByIdList-rest",
      {
        edgeFunctions: { m2mUpdateMealLogByIdList: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLogByIdListRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLogByIdListRestController = (req, res, next) => {
      next(error);
    };
