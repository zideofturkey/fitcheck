...
describe("m2mBulkCreateNutritionDayRestController", function () {
  let functionNameStub;
  let m2mBulkCreateNutritionDayRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateNutritionDayRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateNutritionDay-rest",
      {
        edgeFunctions: { m2mBulkCreateNutritionDay: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateNutritionDayRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateNutritionDayRestController = (req, res, next) => {
      next(error);
    };
