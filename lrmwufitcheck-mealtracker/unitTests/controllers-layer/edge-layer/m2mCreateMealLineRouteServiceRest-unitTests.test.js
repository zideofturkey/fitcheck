...
describe("m2mCreateMealLineRestController", function () {
  let functionNameStub;
  let m2mCreateMealLineRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateMealLineRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateMealLine-rest",
      {
        edgeFunctions: { m2mCreateMealLine: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateMealLineRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateMealLineRestController = (req, res, next) => {
      next(error);
    };
