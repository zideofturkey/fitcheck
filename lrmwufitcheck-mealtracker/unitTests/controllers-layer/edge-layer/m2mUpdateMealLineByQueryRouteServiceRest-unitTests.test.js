...
describe("m2mUpdateMealLineByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateMealLineByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMealLineByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMealLineByQuery-rest",
      {
        edgeFunctions: { m2mUpdateMealLineByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMealLineByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMealLineByQueryRestController = (req, res, next) => {
      next(error);
    };
