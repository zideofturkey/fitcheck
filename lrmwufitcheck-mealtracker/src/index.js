...
console.log("Starting fitcheck-mealtracker-service service...");
...
const start = async () => {
  process.title = "node-lrmwufitcheck-mealtracker-service";
  ...
  try {
    ...
    await setRedisData(
      "lrmwufitcheck-mealtracker-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key lrmwufitcheck-mealtracker-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "fitcheck-mealtracker-service is listening HTTP/REST port " +
...
/* starting the server fitcheck-mealtracker-service .... */
start();
