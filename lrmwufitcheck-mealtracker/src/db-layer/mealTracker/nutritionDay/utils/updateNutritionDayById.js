...
const updateNutritionDayById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await NutritionDay.findOne({ where: { id } });
    ...
    const updateResult = await NutritionDay.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await NutritionDay.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateNutritionDayById;
