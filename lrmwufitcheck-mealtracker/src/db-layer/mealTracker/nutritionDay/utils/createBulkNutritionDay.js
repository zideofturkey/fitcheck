...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: nutritionDayUserDateIdx (doUpdate)
  if (remaining.length > 0) {
    const getKey_nutritionDayUserDateIdx = (r) =>
      String(r.userId ?? null) + "|||" + String(r.summaryDate ?? null);

    const orConditions_nutritionDayUserDateIdx = remaining.map((data) => ({
      userId: data.userId ?? null,
      summaryDate: data.summaryDate ?? null,
    }));

    const conflicts_nutritionDayUserDateIdx = await NutritionDay.findAll({
      where: { [Op.or]: orConditions_nutritionDayUserDateIdx },
    });

    if (conflicts_nutritionDayUserDateIdx.length > 0) {
      const conflictMap_nutritionDayUserDateIdx = new Map();
      conflicts_nutritionDayUserDateIdx.forEach((dbDoc) => {
        conflictMap_nutritionDayUserDateIdx.set(
          getKey_nutritionDayUserDateIdx(dbDoc),
          dbDoc,
        );
      });

      // doUpdate: move matched records to update list
      const next_nutritionDayUserDateIdx = [];
      remaining.forEach((data) => {
        const key = getKey_nutritionDayUserDateIdx(data);
        const dbDoc = conflictMap_nutritionDayUserDateIdx.get(key);
        if (dbDoc && !processedIds.has(dbDoc.id)) {
          processedIds.add(dbDoc.id);
          indexUpdates.push({ data, dbRecord: dbDoc });
        } else {
          next_nutritionDayUserDateIdx.push(data);
        }
      });
      remaining = next_nutritionDayUserDateIdx;
    }
  }
...
const createBulkNutritionDay = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await NutritionDay.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await NutritionDay.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkNutritionDay",
      err,
    );
  }
};

module.exports = createBulkNutritionDay;
