...
/**
 * @deprecated Use getNutritionDayStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getNutritionDayStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(NutritionDay.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            NutritionDay.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            NutritionDay.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            NutritionDay.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            NutritionDay.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await NutritionDay.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingNutritionDayStatsByQuery",
      err,
    );
  }
};

module.exports = getNutritionDayStatsByQuery;
