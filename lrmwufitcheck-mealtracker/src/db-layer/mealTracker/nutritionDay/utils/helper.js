...
const objectName = "nutritionDay";
const serviceCodename = "lrmwufitcheck-mealtracker-service";
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for nutritionDay — no-op
};
 ...
 * on miss" pattern, use getCachedNutritionDayById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for nutritionDay
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new NutritionDayQueryCacheInvalidator();
