...
// This script is called from the TriggerDailyReminderCheckManager

const { NutritionDay } = require("models");

const { NutritionDayQueryCacheInvalidator } = require("./query-cache-classes");
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.nutritionDay;
  return {
    old_nutritionDay: oldData,
    nutritionDay: newData,
...
const dbScriptTriggerDailyremindercheck = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptTriggerDailyremindercheck] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptTriggerDailyremindercheck] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptTriggerDailyremindercheck] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptTriggerDailyremindercheck] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await NutritionDay.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptTriggerDailyremindercheck] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await NutritionDay.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptTriggerDailyremindercheck] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    apiManager.nutritionDay = dbData;
    ...
    const queryCacheInvalidator = new NutritionDayQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptTriggerDailyremindercheck",
      {
...
module.exports = dbScriptTriggerDailyremindercheck;
