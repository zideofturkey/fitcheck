...
// This script is called from the TriggerDailySummaryManager
...
const dbScriptTriggerDailysummary = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptTriggerDailysummary] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptTriggerDailysummary] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptTriggerDailysummary] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptTriggerDailysummary] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptTriggerDailysummary] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await NutritionDay.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptTriggerDailysummary] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptTriggerDailysummary",
      {
...
module.exports = dbScriptTriggerDailysummary;
