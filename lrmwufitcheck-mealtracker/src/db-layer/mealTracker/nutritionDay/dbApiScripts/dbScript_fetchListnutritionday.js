...
// This script is called from the _fetchListNutritionDayManager

const { NutritionDay, sequelize } = require("models");
...
const dbScript_fetchListnutritionday = async (apiManager) => {
  ...
  try {
    ...
    let nutritionDayList = await NutritionDay.findAll(options);

    const dbData = { items: [] };
    apiManager.nutritionDays = [];
    if (!nutritionDayList || nutritionDayList.length == 0) return dbData;

    dbData.items = Array.isArray(nutritionDayList)
      ? nutritionDayList
      : [nutritionDayList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("nutritionDay".id))'), "_COUNT"],
      ];
      let nutritionDay_count = await NutritionDay.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      nutritionDay_count =
        nutritionDay_count && nutritionDay_count.length
          ? nutritionDay_count[0]._COUNT
          : 0;
      nutritionDay_count = isNaN(nutritionDay_count)
        ? nutritionDay_count
        : parseInt(nutritionDay_count);

      nutritionDayList = {
        rows: nutritionDayList,
        count: nutritionDay_count,
      };
      dbData.totalRowCount = nutritionDay_count;
    ...
    apiManager.nutritionDays = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListnutritionday",
      {
...
module.exports = dbScript_fetchListnutritionday;
