...
// This script is called from the UpdateMealLogManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.mealLog;
  return {
    old_mealLog: oldData,
    mealLog: newData,
...
const dbScriptUpdateMeallog = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateMeallog] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateMeallog] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdateMeallog] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateMeallog] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await MealLog.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateMeallog] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await MealLog.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateMeallog] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateMeallog",
      {
...
module.exports = dbScriptUpdateMeallog;
