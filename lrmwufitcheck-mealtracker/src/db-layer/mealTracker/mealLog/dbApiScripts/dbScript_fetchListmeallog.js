...
// This script is called from the _fetchListMealLogManager

const { MealLog, sequelize } = require("models");
...
const dbScript_fetchListmeallog = async (apiManager) => {
  ...
  try {
    ...
    let mealLogList = await MealLog.findAll(options);

    const dbData = { items: [] };
    apiManager.mealLogs = [];
    if (!mealLogList || mealLogList.length == 0) return dbData;

    dbData.items = Array.isArray(mealLogList) ? mealLogList : [mealLogList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("mealLog".id))'), "_COUNT"],
      ];
      let mealLog_count = await MealLog.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      mealLog_count =
        mealLog_count && mealLog_count.length ? mealLog_count[0]._COUNT : 0;
      mealLog_count = isNaN(mealLog_count)
        ? mealLog_count
        : parseInt(mealLog_count);

      mealLogList = {
        rows: mealLogList,
        count: mealLog_count,
      };
      dbData.totalRowCount = mealLog_count;
    ...
    apiManager.mealLogs = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListmeallog",
      {
...
module.exports = dbScript_fetchListmeallog;
