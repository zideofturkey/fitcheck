...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for mealLog — no-op
};
 ...
 * on miss" pattern, use getCachedMealLogById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for mealLog
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new MealLogQueryCacheInvalidator();
