...
/**
 * @deprecated Use getMealLogStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getMealLogStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(MealLog.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            MealLog.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            MealLog.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            MealLog.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            MealLog.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await MealLog.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingMealLogStatsByQuery",
      err,
    );
  }
};

module.exports = getMealLogStatsByQuery;
