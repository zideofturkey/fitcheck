...
const updateMealLogById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MealLog.findOne({ where: { id } });
    ...
    const updateResult = await MealLog.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await MealLog.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateMealLogById;
