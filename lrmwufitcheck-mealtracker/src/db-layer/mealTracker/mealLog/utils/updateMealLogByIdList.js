...
const updateMealLogByIdList = async (idList, dataClause, context = null) => {
  try {
    let rows = [];
    ...
    const mealLogIdList = rows.map((item) => item.id);
    return mealLogIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenUpdatingMealLogByIdList", err);
  }
};

module.exports = updateMealLogByIdList;
