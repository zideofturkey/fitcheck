...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for mealLine — no-op
};
 ...
 * on miss" pattern, use getCachedMealLineById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for mealLine
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new MealLineQueryCacheInvalidator();
