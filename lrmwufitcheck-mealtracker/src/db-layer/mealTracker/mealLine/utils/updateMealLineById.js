...
const updateMealLineById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MealLine.findOne({ where: { id } });
    ...
    const updateResult = await MealLine.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await MealLine.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateMealLineById;
