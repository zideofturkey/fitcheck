...
/**
 * @deprecated Use updateMealLineByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateMealLineByQuery = async (dataClause, query, context = null) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await MealLine.findAll({ where: whereClause });
      ...
      [, rows] = await MealLine.update(dataClause, options);
    }
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenUpdatingMealLineByQuery", err);
  }
};

module.exports = updateMealLineByQuery;
