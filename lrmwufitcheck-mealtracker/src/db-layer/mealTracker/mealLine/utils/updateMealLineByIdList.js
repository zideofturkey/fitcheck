...
const updateMealLineByIdList = async (idList, dataClause, context = null) => {
  try {
    let rows = [];
    ...
    const mealLineIdList = rows.map((item) => item.id);
    return mealLineIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingMealLineByIdList",
      err,
    );
  }
};

module.exports = updateMealLineByIdList;
