...
// This script is called from the _fetchListMealLineManager

const { MealLine, sequelize } = require("models");
...
const dbScript_fetchListmealline = async (apiManager) => {
  ...
  try {
    ...
    let mealLineList = await MealLine.findAll(options);

    const dbData = { items: [] };
    apiManager.mealLines = [];
    if (!mealLineList || mealLineList.length == 0) return dbData;

    dbData.items = Array.isArray(mealLineList) ? mealLineList : [mealLineList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("mealLine".id))'), "_COUNT"],
      ];
      let mealLine_count = await MealLine.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      mealLine_count =
        mealLine_count && mealLine_count.length ? mealLine_count[0]._COUNT : 0;
      mealLine_count = isNaN(mealLine_count)
        ? mealLine_count
        : parseInt(mealLine_count);

      mealLineList = {
        rows: mealLineList,
        count: mealLine_count,
      };
      dbData.totalRowCount = mealLine_count;
    ...
    apiManager.mealLines = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListmealline",
      {
...
module.exports = dbScript_fetchListmealline;
