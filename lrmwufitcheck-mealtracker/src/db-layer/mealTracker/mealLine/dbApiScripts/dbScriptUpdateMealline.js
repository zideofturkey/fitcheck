...
// This script is called from the UpdateMealLineManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.mealLine;
  return {
    old_mealLine: oldData,
    mealLine: newData,
...
const dbScriptUpdateMealline = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateMealline] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateMealline] whereClause:",
      require("util").inspect(whereClause, { depth: 4 }),
    );

    const updateResult = await MealLine.update(resolvedDataClause, {
    ...
    console.log(
      "[DbScript:dbScriptUpdateMealline] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateMealline] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await MealLine.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateMealline] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await MealLine.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateMealline] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateMealline",
      {
...
module.exports = dbScriptUpdateMealline;
