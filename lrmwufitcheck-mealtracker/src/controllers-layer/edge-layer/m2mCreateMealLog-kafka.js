...
const { m2mCreateMealLog } = require("edgeFunctions");

const m2mCreateMealLogHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mCreateMealLogInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mCreateMealLogRequiresM2MToken",
    ...
    const result = await m2mCreateMealLog(message);
    // log result
  } catch (err) {
    console.error("Error running routeService for m2mCreateMealLog: ", err);
    // log error
  }
};

module.exports = m2mCreateMealLogHandler;
