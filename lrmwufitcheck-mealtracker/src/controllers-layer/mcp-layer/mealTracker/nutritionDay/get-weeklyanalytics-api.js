...
class GetWeeklyAnalyticsMcpController extends MealTrackerServiceMcpController {
  constructor(params) {
    super("getWeeklyAnalytics", "getweeklyanalytics", params);
    this.dataName = "nutritionDays";
    this.crudType = "list";
  }

  createApiManager() {
    return new GetWeeklyAnalyticsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: GetWeeklyAnalyticsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetWeeklyAnalyticsMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetWeeklyAnalyticsMcpController.getOutputSchema().parse(result);
