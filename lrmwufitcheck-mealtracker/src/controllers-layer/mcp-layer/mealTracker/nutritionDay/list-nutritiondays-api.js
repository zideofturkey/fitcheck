...
class ListNutritionDaysMcpController extends MealTrackerServiceMcpController {
  constructor(params) {
    super("listNutritionDays", "listnutritiondays", params);
    this.dataName = "nutritionDays";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListNutritionDaysManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      fromDate: z.string().datetime().optional().describe("Range start"),

      toDate: z.string().datetime().optional().describe("Range end"),
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: ListNutritionDaysMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListNutritionDaysMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListNutritionDaysMcpController.getOutputSchema().parse(result);
