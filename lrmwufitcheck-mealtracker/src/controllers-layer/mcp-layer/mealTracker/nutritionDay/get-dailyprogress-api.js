...
class GetDailyProgressMcpController extends MealTrackerServiceMcpController {
  constructor(params) {
    super("getDailyProgress", "getdailyprogress", params);
    this.dataName = "nutritionDay";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetDailyProgressManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      targetDate: z
        ...
        .datetime()
        .optional()
        .describe("The day to retrieve progress for; defaults to today"),
    };
  }
}
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: GetDailyProgressMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetDailyProgressMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetDailyProgressMcpController.getOutputSchema().parse(result);
