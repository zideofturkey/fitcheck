# Business API Design Specification - `List Invitelinks`

A Business API is a set of logical actions centered around a main data object. These actions can range from simple CRUD operations to complex workflows that implement intricate business logic.

While the term “API” traditionally refers to an interface that allows software systems to interact, in Mindbricks a Business API represents a broader concept. It encapsulates a business workflow around a data object, going beyond basic CRUD operations to include rich, internally coordinated actions that can be fully designed and customized.

This document provides an in-depth explanation of the architectural design of the `listInviteLinks` Business API. It is intended to guide backend architects and developers in maintaining the current design. Additionally, frontend developers and frontend AI agents can use this document to understand how to properly consume this API on the client side.

## Main Data Object and CRUD Operation

The `listInviteLinks` Business API is designed to handle a `list` operation on the `InviteLink` data object. This operation is performed under the specified conditions and may include additional, coordinated actions as part of the workflow.
...
The `listInviteLinks` Business API includes a REST controller that can be triggered via the following route:
...
REST controllers also expose the Business API as a tool in the MCP, making it accessible to AI agents. This `listInviteLinks` Business API will be registered as a tool on the MCP server within the service binding.

## API Parameters

The `listInviteLinks` Business API has 2 parameters that must be sent from the controller. Note that all parameters, except session and Redis parameters, should be provided by the client.
...
The `listInviteLinks` api supports 2 optional filter parameters for filtering list results using URL query parameters. These parameters are only available for `list` type APIs.

#### `usageMode` Filter

**Type:** `Enum`  
**Description:** Filter by usageMode  
**Location:** Query Parameter

**Usage:**

- Single value: `?usageMode=<value>` (case-insensitive)
- Multiple values: `?usageMode=<value1>&usageMode=<value2>`
- Null check: `?usageMode=null`

**Examples:**

```javascript
// Get records with specific enum value
GET /v1/invite-links?usageMode=active

// Get records with multiple enum values (use multiple parameters)
GET /v1/invite-links?usageMode=active&usageMode=pending

// Get records without this field
GET /v1/invite-links?usageMode=null
```

#### `inviteState` Filter

**Type:** `Enum`  
**Description:** Filter by inviteState  
**Location:** Query Parameter

**Usage:**

- Single value: `?inviteState=<value>` (case-insensitive)
- Multiple values: `?inviteState=<value1>&inviteState=<value2>`
- Null check: `?inviteState=null`

**Examples:**

```javascript
// Get records with specific enum value
GET /v1/invite-links?inviteState=active

// Get records with multiple enum values (use multiple parameters)
GET /v1/invite-links?inviteState=active&inviteState=pending

// Get records without this field
GET /v1/invite-links?inviteState=null
```
...
The **authentication and authorization configuration** defines the core access rules for the `listInviteLinks` Business API. These checks are applied **after parameter validation** and before executing the main business logic.
...
The `listInviteLinks` api has 2 filter parameters available for filtering list results. See the [Filter Parameters](#filter-parameters) section above for detailed usage examples.

| Filter Parameter | Type | Array Property | Description           |
| ---------------- | ---- | -------------- | --------------------- |
| usageMode        | Enum | No             | Filter by usageMode   |
| inviteState      | Enum | No             | Filter by inviteState |
...
Following JSON represents the most comprehensive form of the **`inviteLinks`** object in the respones. However, some properties may be omitted based on the object's internal logic.
