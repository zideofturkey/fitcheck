...
describe("m2mUpdateInviteAuditByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateInviteAuditByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateInviteAuditByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateInviteAuditById-rest",
      {
        edgeFunctions: { m2mUpdateInviteAuditById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateInviteAuditByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateInviteAuditByIdRestController = (req, res, next) => {
      next(error);
    };
