...
describe("m2mCreateInviteAuditRestController", function () {
  let functionNameStub;
  let m2mCreateInviteAuditRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateInviteAuditRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateInviteAudit-rest",
      {
        edgeFunctions: { m2mCreateInviteAudit: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateInviteAuditRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateInviteAuditRestController = (req, res, next) => {
      next(error);
    };
