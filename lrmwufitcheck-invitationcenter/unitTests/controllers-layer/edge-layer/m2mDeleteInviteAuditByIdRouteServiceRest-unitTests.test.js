...
describe("m2mDeleteInviteAuditByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteInviteAuditByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteInviteAuditByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteInviteAuditById-rest",
      {
        edgeFunctions: { m2mDeleteInviteAuditById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteInviteAuditByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteInviteAuditByIdRestController = (req, res, next) => {
      next(error);
    };
