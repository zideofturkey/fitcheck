...
describe("m2mBulkCreateInviteLinkRestController", function () {
  let functionNameStub;
  let m2mBulkCreateInviteLinkRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateInviteLinkRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateInviteLink-rest",
      {
        edgeFunctions: { m2mBulkCreateInviteLink: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateInviteLinkRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateInviteLinkRestController = (req, res, next) => {
      next(error);
    };
