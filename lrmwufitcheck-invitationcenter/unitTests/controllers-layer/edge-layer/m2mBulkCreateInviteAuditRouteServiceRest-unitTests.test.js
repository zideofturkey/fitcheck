...
describe("m2mBulkCreateInviteAuditRestController", function () {
  let functionNameStub;
  let m2mBulkCreateInviteAuditRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateInviteAuditRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateInviteAudit-rest",
      {
        edgeFunctions: { m2mBulkCreateInviteAudit: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateInviteAuditRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateInviteAuditRestController = (req, res, next) => {
      next(error);
    };
