...
describe("m2mUpdateInviteLinkByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateInviteLinkByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateInviteLinkByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateInviteLinkByQuery-rest",
      {
        edgeFunctions: { m2mUpdateInviteLinkByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateInviteLinkByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateInviteLinkByQueryRestController = (req, res, next) => {
      next(error);
    };
