...
describe("m2mDeleteInviteLinkByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteInviteLinkByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteInviteLinkByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteInviteLinkById-rest",
      {
        edgeFunctions: { m2mDeleteInviteLinkById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteInviteLinkByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteInviteLinkByIdRestController = (req, res, next) => {
      next(error);
    };
