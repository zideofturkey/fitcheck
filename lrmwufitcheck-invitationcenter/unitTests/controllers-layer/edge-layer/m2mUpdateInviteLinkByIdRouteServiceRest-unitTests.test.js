...
describe("m2mUpdateInviteLinkByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateInviteLinkByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateInviteLinkByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateInviteLinkById-rest",
      {
        edgeFunctions: { m2mUpdateInviteLinkById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateInviteLinkByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateInviteLinkByIdRestController = (req, res, next) => {
      next(error);
    };
