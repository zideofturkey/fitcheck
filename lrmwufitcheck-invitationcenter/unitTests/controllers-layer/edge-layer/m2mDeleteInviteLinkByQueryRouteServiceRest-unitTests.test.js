...
describe("m2mDeleteInviteLinkByQueryRestController", function () {
  let functionNameStub;
  let m2mDeleteInviteLinkByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteInviteLinkByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteInviteLinkByQuery-rest",
      {
        edgeFunctions: { m2mDeleteInviteLinkByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteInviteLinkByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteInviteLinkByQueryRestController = (req, res, next) => {
      next(error);
    };
