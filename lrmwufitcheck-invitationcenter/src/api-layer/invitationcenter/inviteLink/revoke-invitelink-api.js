...
const { dbScriptRevokeInvitelink, createInviteAudit } = require("dbLayer");
...
class RevokeInviteLinkManager extends InviteLinkManager {
  ...
  parametersToJson(jsonObj) {
    ...
    jsonObj.eventNote = this.eventNote;
  ...
  readRestParameters(request) {
    ...
    this.eventNote = request.body?.["eventNote"];
  ...
  readMcpParameters(request) {
    ...
    this.eventNote = request.mcpParams?.["eventNote"];
    this.ownerUserId = request.session?.["userId"];
  ...
  async getRouteQuery() {
    return runMScript(
      () => ({
        ...
        inviteState: { $in: ["draft", "active"] },
      }),
      { path: "services[1].businessLogic[2].whereClause.fullWhereClause" },
    );
  ...
  checkParameter_eventNote() {
    if (this.eventNote == null) return;

    if (Array.isArray(this.eventNote)) {
      throw new BadRequestError("errMsg_eventNoteMustNotBeAnArray");
    }
  ...
  async executeMainOperation() {
    return await dbScriptRevokeInvitelink(this);
  ...
  async afterFetchInstance() {
    try {
      await this.checkCanRevoke();
    } catch (err) {
      console.log("checkCanRevoke Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  async afterMainUpdateOperation() {
    try {
      await this.createAuditOnRevoke();
    } catch (err) {
      console.log("createAuditOnRevoke Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  /***********************************************************************
   ** Insert an inviteAudit row recording the 'revoked' lifecycle event
   ***********************************************************************/
  async createAuditOnRevoke() {
    // Aggregated Update Operation on childObject inviteAudit

    const params = {
      inviteLinkId: runMScript(() => this.inviteLinkId, {
        path: "services[1].businessLogic[2].actions.createCrudActions[0].dataClause[0].dataValue",
      }),
      eventType: "revoked",
      eventAt: runMScript(() => LIB.now(), {
        path: "services[1].businessLogic[2].actions.createCrudActions[0].dataClause[2].dataValue",
      }),
      actorUserId: runMScript(() => this.session.userId, {
        path: "services[1].businessLogic[2].actions.createCrudActions[0].dataClause[3].dataValue",
      }),
      eventNote: runMScript(() => this.eventNote, {
        path: "services[1].businessLogic[2].actions.createCrudActions[0].dataClause[4].dataValue",
      }),
    };
  ...
  /***********************************************************************
   ** Verify the invite link is in draft or active state before revoking
   ***********************************************************************/

  async checkCanRevoke() {
    let isValid;
    try {
      isValid = runMScript(
        () =>
          this.inviteLink &&
          (this.inviteLink.inviteState === "draft" ||
            this.inviteLink.inviteState === "active"),
        {
          path: "services[1].businessLogic[2].actions.validationActions[0].validationScript",
        },
      );
      ...
      throw new HttpServerError(
        `Validation 'checkCanRevoke' script failed: ${err.message}`,
        err,
      );
    }
    ...
    if (!isValid) {
      throw new BadRequestError(
        "Invite link cannot be revoked in its current state",
      );
    }
    return isValid;
  }
...
module.exports = RevokeInviteLinkManager;
