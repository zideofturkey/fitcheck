...
const { dbScriptActivateInvitelink, createInviteAudit } = require("dbLayer");
...
class ActivateInviteLinkManager extends InviteLinkManager {
  ...
  async getRouteQuery() {
    return runMScript(() => ({ id: this.inviteLinkId, inviteState: "draft" }), {
      path: "services[1].businessLogic[1].whereClause.fullWhereClause",
    });
  ...
  async executeMainOperation() {
    return await dbScriptActivateInvitelink(this);
  ...
  async afterFetchInstance() {
    try {
      await this.checkIsDraft();
    } catch (err) {
      console.log("checkIsDraft Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  async afterMainUpdateOperation() {
    try {
      await this.createAuditOnActivate();
    } catch (err) {
      console.log("createAuditOnActivate Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  /***********************************************************************
   ** Insert an inviteAudit row recording the 'activated' lifecycle event
   ***********************************************************************/
  async createAuditOnActivate() {
    // Aggregated Update Operation on childObject inviteAudit

    const params = {
      inviteLinkId: runMScript(() => this.inviteLinkId, {
        path: "services[1].businessLogic[1].actions.createCrudActions[0].dataClause[0].dataValue",
      }),
      eventType: "activated",
      eventAt: runMScript(() => LIB.now(), {
        path: "services[1].businessLogic[1].actions.createCrudActions[0].dataClause[2].dataValue",
      }),
      actorUserId: runMScript(() => this.session.userId, {
        path: "services[1].businessLogic[1].actions.createCrudActions[0].dataClause[3].dataValue",
      }),
    };
  ...
  /***********************************************************************
   ** Verify the invite link was found and is in draft state before
   ** activating
   ***********************************************************************/
  ...
  async checkIsDraft() {
    ...
    try {
      isValid = runMScript(
        () => this.inviteLink && this.inviteLink.inviteState === "draft",
        {
      ...
      throw new HttpServerError(
        `Validation 'checkIsDraft' script failed: ${err.message}`,
        err,
      );
    }

    if (!isValid) {
      throw new BadRequestError("Invite link is not in draft state");
    }
    return isValid;
  }
}

module.exports = ActivateInviteLinkManager;
