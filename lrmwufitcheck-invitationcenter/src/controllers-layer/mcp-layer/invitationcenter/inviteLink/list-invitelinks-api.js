...
class ListInviteLinksMcpController extends InvitationCenterServiceMcpController {
  constructor(params) {
    super("listInviteLinks", "listinvitelinks", params);
    this.dataName = "inviteLinks";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListInviteLinksManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: ListInviteLinksMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListInviteLinksMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListInviteLinksMcpController.getOutputSchema().parse(result);
