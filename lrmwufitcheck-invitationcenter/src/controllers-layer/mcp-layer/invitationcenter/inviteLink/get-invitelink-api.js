...
class GetInviteLinkMcpController extends InvitationCenterServiceMcpController {
  constructor(params) {
    super("getInviteLink", "getinvitelink", params);
    this.dataName = "inviteLink";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetInviteLinkManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: GetInviteLinkMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetInviteLinkMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetInviteLinkMcpController.getOutputSchema().parse(result);
