...
const { m2mCreateInviteLink } = require("edgeFunctions");

const m2mCreateInviteLinkHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mCreateInviteLinkInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mCreateInviteLinkRequiresM2MToken",
    ...
    const result = await m2mCreateInviteLink(message);
    // log result
  } catch (err) {
    console.error("Error running routeService for m2mCreateInviteLink: ", err);
    // log error
  }
};

module.exports = m2mCreateInviteLinkHandler;
