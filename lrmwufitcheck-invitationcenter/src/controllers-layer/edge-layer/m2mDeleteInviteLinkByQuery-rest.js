...
const { m2mDeleteInviteLinkByQuery } = require("edgeFunctions");
const createServiceController = require("restLayer/create-service-controller");

const m2mDeleteInviteLinkByQueryRestController = async (req, res, next) => {
  try {
    req.loginRequired = false;
    const statusCode = 200;

    const restController = createServiceController(
      "m2mDeleteInviteLinkByQuery",
      "m2mDeleteInviteLinkByQuery",
      req,
      res,
    );
    ...
    if (req.M2MToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mDeleteInviteLinkByQueryInvalidM2MToken",
    ...
    const result = await m2mDeleteInviteLinkByQuery(req);
    ...
    console.error(
      "Error running routeService for m2mDeleteInviteLinkByQuery: ",
      err,
    );
    return next(err);
  }
...
module.exports = m2mDeleteInviteLinkByQueryRestController;
