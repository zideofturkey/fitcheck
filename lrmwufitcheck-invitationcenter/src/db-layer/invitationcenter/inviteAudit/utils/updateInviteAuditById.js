...
const updateInviteAuditById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InviteAudit.findOne({ where: { id } });
    ...
    const updateResult = await InviteAudit.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await InviteAudit.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateInviteAuditById;
