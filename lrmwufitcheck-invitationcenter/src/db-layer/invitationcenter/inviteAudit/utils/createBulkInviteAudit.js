...
const createBulkInviteAudit = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await InviteAudit.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await InviteAudit.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenCreatingBulkInviteAudit", err);
  }
};

module.exports = createBulkInviteAudit;
