...
const objectName = "inviteAudit";
const serviceCodename = "lrmwufitcheck-invitationcenter-service";
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for inviteAudit — no-op
};
 ...
 * on miss" pattern, use getCachedInviteAuditById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for inviteAudit
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new InviteAuditQueryCacheInvalidator();
