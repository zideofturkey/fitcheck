...
/**
 * @deprecated Use getInviteAuditStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getInviteAuditStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(InviteAudit.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            InviteAudit.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            InviteAudit.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            InviteAudit.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            InviteAudit.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await InviteAudit.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingInviteAuditStatsByQuery",
      err,
    );
  }
};

module.exports = getInviteAuditStatsByQuery;
