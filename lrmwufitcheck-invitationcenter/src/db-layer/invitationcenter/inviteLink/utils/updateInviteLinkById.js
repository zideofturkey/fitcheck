...
const updateInviteLinkById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InviteLink.findOne({ where: { id } });
    ...
    const updateResult = await InviteLink.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await InviteLink.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateInviteLinkById;
