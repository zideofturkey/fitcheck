...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for inviteLink — no-op
};
 ...
 * on miss" pattern, use getCachedInviteLinkById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for inviteLink
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new InviteLinkQueryCacheInvalidator();
