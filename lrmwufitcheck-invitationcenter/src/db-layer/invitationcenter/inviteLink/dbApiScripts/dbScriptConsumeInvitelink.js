...
// This script is called from the ConsumeInviteLinkManager
...
const dbScriptConsumeInvitelink = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptConsumeInvitelink] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptConsumeInvitelink] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptConsumeInvitelink] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptConsumeInvitelink] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptConsumeInvitelink] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptConsumeInvitelink] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptConsumeInvitelink",
      {
...
module.exports = dbScriptConsumeInvitelink;
