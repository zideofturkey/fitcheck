...
// This script is called from the RevokeInviteLinkManager
...
const dbScriptRevokeInvitelink = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptRevokeInvitelink] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptRevokeInvitelink] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptRevokeInvitelink] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptRevokeInvitelink] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptRevokeInvitelink] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptRevokeInvitelink] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptRevokeInvitelink",
      {
...
module.exports = dbScriptRevokeInvitelink;
