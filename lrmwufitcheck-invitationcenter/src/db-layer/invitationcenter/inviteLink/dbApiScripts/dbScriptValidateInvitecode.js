...
// This script is called from the ValidateInviteCodeManager
...
const dbScriptValidateInvitecode = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptValidateInvitecode] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptValidateInvitecode] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptValidateInvitecode] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptValidateInvitecode] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptValidateInvitecode] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptValidateInvitecode] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptValidateInvitecode",
      {
...
module.exports = dbScriptValidateInvitecode;
