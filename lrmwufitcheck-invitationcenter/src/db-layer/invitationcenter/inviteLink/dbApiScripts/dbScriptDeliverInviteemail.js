...
// This script is called from the DeliverInviteEmailManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.inviteLink;
  return {
    old_inviteLink: oldData,
    inviteLink: newData,
...
const dbScriptDeliverInviteemail = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptDeliverInviteemail] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptDeliverInviteemail] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptDeliverInviteemail] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptDeliverInviteemail] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptDeliverInviteemail] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptDeliverInviteemail] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeliverInviteemail",
      {
...
module.exports = dbScriptDeliverInviteemail;
