...
// This script is called from the _fetchListInviteLinkManager

const { InviteLink, sequelize } = require("models");
...
const dbScript_fetchListinvitelink = async (apiManager) => {
  ...
  try {
    ...
    let inviteLinkList = await InviteLink.findAll(options);

    const dbData = { items: [] };
    apiManager.inviteLinks = [];
    if (!inviteLinkList || inviteLinkList.length == 0) return dbData;

    dbData.items = Array.isArray(inviteLinkList)
      ? inviteLinkList
      : [inviteLinkList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("inviteLink".id))'), "_COUNT"],
      ];
      let inviteLink_count = await InviteLink.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      inviteLink_count =
        inviteLink_count && inviteLink_count.length
          ? inviteLink_count[0]._COUNT
          : 0;
      inviteLink_count = isNaN(inviteLink_count)
        ? inviteLink_count
        : parseInt(inviteLink_count);

      inviteLinkList = {
        rows: inviteLinkList,
        count: inviteLink_count,
      };
      dbData.totalRowCount = inviteLink_count;
    ...
    apiManager.inviteLinks = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListinvitelink",
      {
...
module.exports = dbScript_fetchListinvitelink;
