...
// This script is called from the ActivateInviteLinkManager
...
const dbScriptActivateInvitelink = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptActivateInvitelink] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptActivateInvitelink] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptActivateInvitelink] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptActivateInvitelink] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptActivateInvitelink] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await InviteLink.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptActivateInvitelink] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptActivateInvitelink",
      {
...
module.exports = dbScriptActivateInvitelink;
