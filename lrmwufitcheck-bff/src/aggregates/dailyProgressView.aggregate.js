...
const dailyProgressViewAggregateData = async (id) => {
  try {
    ...
    for (const hit of result?.hits?.hits) {
      ...
      await elasticClient.index({
        index: "lrmwufitcheck_dailyprogressview",
        id: source["id"],
        body: source,
      });
    }
...
const userProfileReDailyProgressView = async (id) => {
  try {
    const idList = Array.isArray(id) ? id : [id];
    ...
    console.log("Error in userReAggregatedailyProgressView", error);
    //**errorLog
  }
};
