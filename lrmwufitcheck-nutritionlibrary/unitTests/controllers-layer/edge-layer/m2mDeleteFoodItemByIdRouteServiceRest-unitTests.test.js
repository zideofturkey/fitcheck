...
describe("m2mDeleteFoodItemByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteFoodItemByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteFoodItemByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteFoodItemById-rest",
      {
        edgeFunctions: { m2mDeleteFoodItemById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteFoodItemByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteFoodItemByIdRestController = (req, res, next) => {
      next(error);
    };
