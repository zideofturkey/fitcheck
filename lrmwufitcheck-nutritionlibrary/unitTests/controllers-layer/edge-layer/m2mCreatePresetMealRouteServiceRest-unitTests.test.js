...
describe("m2mCreatePresetMealRestController", function () {
  let functionNameStub;
  let m2mCreatePresetMealRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreatePresetMealRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreatePresetMeal-rest",
      {
        edgeFunctions: { m2mCreatePresetMeal: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreatePresetMealRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreatePresetMealRestController = (req, res, next) => {
      next(error);
    };
