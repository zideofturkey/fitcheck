...
describe("m2mUpdatePresetMealByIdRestController", function () {
  let functionNameStub;
  let m2mUpdatePresetMealByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdatePresetMealByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdatePresetMealById-rest",
      {
        edgeFunctions: { m2mUpdatePresetMealById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdatePresetMealByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdatePresetMealByIdRestController = (req, res, next) => {
      next(error);
    };
