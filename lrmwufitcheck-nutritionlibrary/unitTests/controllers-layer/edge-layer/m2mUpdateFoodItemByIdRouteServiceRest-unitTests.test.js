...
describe("m2mUpdateFoodItemByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateFoodItemByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateFoodItemByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateFoodItemById-rest",
      {
        edgeFunctions: { m2mUpdateFoodItemById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateFoodItemByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateFoodItemByIdRestController = (req, res, next) => {
      next(error);
    };
