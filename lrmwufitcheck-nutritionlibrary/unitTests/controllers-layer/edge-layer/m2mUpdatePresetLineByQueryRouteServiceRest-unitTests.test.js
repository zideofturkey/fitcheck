...
describe("m2mUpdatePresetLineByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdatePresetLineByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdatePresetLineByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdatePresetLineByQuery-rest",
      {
        edgeFunctions: { m2mUpdatePresetLineByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdatePresetLineByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdatePresetLineByQueryRestController = (req, res, next) => {
      next(error);
    };
