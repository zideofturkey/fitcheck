...
describe("m2mUpdatePresetMealByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdatePresetMealByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdatePresetMealByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdatePresetMealByQuery-rest",
      {
        edgeFunctions: { m2mUpdatePresetMealByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdatePresetMealByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdatePresetMealByQueryRestController = (req, res, next) => {
      next(error);
    };
