...
describe("m2mDeletePresetMealByIdRestController", function () {
  let functionNameStub;
  let m2mDeletePresetMealByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeletePresetMealByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeletePresetMealById-rest",
      {
        edgeFunctions: { m2mDeletePresetMealById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeletePresetMealByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeletePresetMealByIdRestController = (req, res, next) => {
      next(error);
    };
