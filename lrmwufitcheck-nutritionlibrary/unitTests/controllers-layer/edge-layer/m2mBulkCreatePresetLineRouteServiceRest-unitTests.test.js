...
describe("m2mBulkCreatePresetLineRestController", function () {
  let functionNameStub;
  let m2mBulkCreatePresetLineRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreatePresetLineRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreatePresetLine-rest",
      {
        edgeFunctions: { m2mBulkCreatePresetLine: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreatePresetLineRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreatePresetLineRestController = (req, res, next) => {
      next(error);
    };
