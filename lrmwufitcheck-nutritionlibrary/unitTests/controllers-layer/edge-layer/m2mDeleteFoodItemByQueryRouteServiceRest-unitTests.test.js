...
describe("m2mDeleteFoodItemByQueryRestController", function () {
  let functionNameStub;
  let m2mDeleteFoodItemByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteFoodItemByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteFoodItemByQuery-rest",
      {
        edgeFunctions: { m2mDeleteFoodItemByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteFoodItemByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteFoodItemByQueryRestController = (req, res, next) => {
      next(error);
    };
