...
describe("m2mUpdatePresetLineByIdRestController", function () {
  let functionNameStub;
  let m2mUpdatePresetLineByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdatePresetLineByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdatePresetLineById-rest",
      {
        edgeFunctions: { m2mUpdatePresetLineById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdatePresetLineByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdatePresetLineByIdRestController = (req, res, next) => {
      next(error);
    };
