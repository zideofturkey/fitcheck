...
describe("m2mCreatePresetLineRestController", function () {
  let functionNameStub;
  let m2mCreatePresetLineRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreatePresetLineRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreatePresetLine-rest",
      {
        edgeFunctions: { m2mCreatePresetLine: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreatePresetLineRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreatePresetLineRestController = (req, res, next) => {
      next(error);
    };
