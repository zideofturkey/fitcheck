...
describe("m2mUpdateFoodItemByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateFoodItemByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateFoodItemByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateFoodItemByQuery-rest",
      {
        edgeFunctions: { m2mUpdateFoodItemByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateFoodItemByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateFoodItemByQueryRestController = (req, res, next) => {
      next(error);
    };
