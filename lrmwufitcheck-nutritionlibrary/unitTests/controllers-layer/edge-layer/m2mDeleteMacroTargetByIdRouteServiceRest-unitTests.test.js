...
describe("m2mDeleteMacroTargetByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteMacroTargetByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteMacroTargetByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteMacroTargetById-rest",
      {
        edgeFunctions: { m2mDeleteMacroTargetById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteMacroTargetByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteMacroTargetByIdRestController = (req, res, next) => {
      next(error);
    };
