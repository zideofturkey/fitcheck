...
describe("m2mCreateFoodItemRestController", function () {
  let functionNameStub;
  let m2mCreateFoodItemRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateFoodItemRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateFoodItem-rest",
      {
        edgeFunctions: { m2mCreateFoodItem: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateFoodItemRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateFoodItemRestController = (req, res, next) => {
      next(error);
    };
