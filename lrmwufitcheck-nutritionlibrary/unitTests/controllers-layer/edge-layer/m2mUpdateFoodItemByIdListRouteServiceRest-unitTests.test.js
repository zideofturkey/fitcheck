...
describe("m2mUpdateFoodItemByIdListRestController", function () {
  let functionNameStub;
  let m2mUpdateFoodItemByIdListRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateFoodItemByIdListRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateFoodItemByIdList-rest",
      {
        edgeFunctions: { m2mUpdateFoodItemByIdList: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateFoodItemByIdListRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateFoodItemByIdListRestController = (req, res, next) => {
      next(error);
    };
