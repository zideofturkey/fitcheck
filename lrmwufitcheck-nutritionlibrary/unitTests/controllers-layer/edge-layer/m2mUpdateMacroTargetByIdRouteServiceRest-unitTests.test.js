...
describe("m2mUpdateMacroTargetByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateMacroTargetByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateMacroTargetByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateMacroTargetById-rest",
      {
        edgeFunctions: { m2mUpdateMacroTargetById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateMacroTargetByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateMacroTargetByIdRestController = (req, res, next) => {
      next(error);
    };
