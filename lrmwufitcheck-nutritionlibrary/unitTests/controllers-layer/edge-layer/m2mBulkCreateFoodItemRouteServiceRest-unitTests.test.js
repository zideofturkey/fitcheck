...
describe("m2mBulkCreateFoodItemRestController", function () {
  let functionNameStub;
  let m2mBulkCreateFoodItemRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateFoodItemRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateFoodItem-rest",
      {
        edgeFunctions: { m2mBulkCreateFoodItem: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateFoodItemRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateFoodItemRestController = (req, res, next) => {
      next(error);
    };
