...
describe("m2mDeletePresetMealByQueryRestController", function () {
  let functionNameStub;
  let m2mDeletePresetMealByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeletePresetMealByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeletePresetMealByQuery-rest",
      {
        edgeFunctions: { m2mDeletePresetMealByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeletePresetMealByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeletePresetMealByQueryRestController = (req, res, next) => {
      next(error);
    };
