...
describe("m2mCreateMacroTargetRestController", function () {
  let functionNameStub;
  let m2mCreateMacroTargetRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateMacroTargetRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateMacroTarget-rest",
      {
        edgeFunctions: { m2mCreateMacroTarget: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateMacroTargetRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateMacroTargetRestController = (req, res, next) => {
      next(error);
    };
