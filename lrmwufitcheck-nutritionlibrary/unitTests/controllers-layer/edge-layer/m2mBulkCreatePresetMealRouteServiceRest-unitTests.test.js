...
describe("m2mBulkCreatePresetMealRestController", function () {
  let functionNameStub;
  let m2mBulkCreatePresetMealRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreatePresetMealRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreatePresetMeal-rest",
      {
        edgeFunctions: { m2mBulkCreatePresetMeal: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreatePresetMealRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreatePresetMealRestController = (req, res, next) => {
      next(error);
    };
