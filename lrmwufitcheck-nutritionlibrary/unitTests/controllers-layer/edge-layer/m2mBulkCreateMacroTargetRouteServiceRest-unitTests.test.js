...
describe("m2mBulkCreateMacroTargetRestController", function () {
  let functionNameStub;
  let m2mBulkCreateMacroTargetRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateMacroTargetRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateMacroTarget-rest",
      {
        edgeFunctions: { m2mBulkCreateMacroTarget: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateMacroTargetRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateMacroTargetRestController = (req, res, next) => {
      next(error);
    };
