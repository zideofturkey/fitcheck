...
describe("m2mDeletePresetLineByQueryRestController", function () {
  let functionNameStub;
  let m2mDeletePresetLineByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeletePresetLineByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeletePresetLineByQuery-rest",
      {
        edgeFunctions: { m2mDeletePresetLineByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeletePresetLineByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeletePresetLineByQueryRestController = (req, res, next) => {
      next(error);
    };
