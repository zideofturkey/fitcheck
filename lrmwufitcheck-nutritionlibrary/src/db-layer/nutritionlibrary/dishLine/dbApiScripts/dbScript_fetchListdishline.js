...
// This script is called from the _fetchListDishLineManager

const { DishLine, sequelize } = require("models");
...
const dbScript_fetchListdishline = async (apiManager) => {
  ...
  try {
    ...
    let dishLineList = await DishLine.findAll(options);

    const dbData = { items: [] };
    apiManager.dishLines = [];
    if (!dishLineList || dishLineList.length == 0) return dbData;

    dbData.items = Array.isArray(dishLineList)
      ? dishLineList
      : [dishLineList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("dishLine".id))'), "_COUNT"],
      ];
      let dishLine_count = await DishLine.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      dishLine_count =
        dishLine_count && dishLine_count.length
          ? dishLine_count[0]._COUNT
          : 0;
      dishLine_count = isNaN(dishLine_count)
        ? dishLine_count
        : parseInt(dishLine_count);

      dishLineList = {
        rows: dishLineList,
        count: dishLine_count,
      };
      dbData.totalRowCount = dishLine_count;
    ...
    apiManager.dishLines = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListdishline",
      {
...
module.exports = dbScript_fetchListdishline;
