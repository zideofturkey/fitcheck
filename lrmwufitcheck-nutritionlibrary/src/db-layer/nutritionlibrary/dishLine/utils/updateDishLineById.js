...
const updateDishLineById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await DishLine.findOne({ where });
    ...
    const updateResult = await DishLine.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await DishLine.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateDishLineById;
