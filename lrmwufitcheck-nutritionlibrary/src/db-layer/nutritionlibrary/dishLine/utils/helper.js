...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for dishLine — no-op
};
 ...
 * on miss" pattern, use getCachedDishLineById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for dishLine
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new DishLineQueryCacheInvalidator();
