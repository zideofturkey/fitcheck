...
/**
 * @deprecated Use getDishLineStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getDishLineStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(DishLine.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            DishLine.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            DishLine.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            DishLine.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            DishLine.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await DishLine.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingDishLineStatsByQuery",
      err,
    );
  }
};

module.exports = getDishLineStatsByQuery;
