...
/**
 * @deprecated Use updateDishLineByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateDishLineByQuery = async (dataClause, query, context = null) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await DishLine.findAll({ where: whereClause });
      for (const item of existingRows) {
        const resolvedDataClause = resolveArrayMutationsInClause(
          dataClause,
          item.getData(),
        );
        const updateResult = await DishLine.update(resolvedDataClause, {
          where: { id: item.id },
          returning: true,
        });
      ...
      [, rows] = await DishLine.update(dataClause, options);
    }
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingDishLineByQuery",
      err,
    );
  }
};

module.exports = updateDishLineByQuery;
