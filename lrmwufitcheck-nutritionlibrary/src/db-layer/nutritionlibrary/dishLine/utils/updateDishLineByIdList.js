...
const updateDishLineByIdList = async (idList, dataClause, context = null) => {
  try {
    let rows = [];
    ...
    const dishLineIdList = rows.map((item) => item.id);
    return dishLineIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingDishLineByIdList",
      err,
    );
  }
};

module.exports = updateDishLineByIdList;
