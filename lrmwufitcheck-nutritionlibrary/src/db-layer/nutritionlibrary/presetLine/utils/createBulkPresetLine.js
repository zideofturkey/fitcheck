...
const createBulkPresetLine = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await PresetLine.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await PresetLine.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenCreatingBulkPresetLine", err);
  }
};

module.exports = createBulkPresetLine;
