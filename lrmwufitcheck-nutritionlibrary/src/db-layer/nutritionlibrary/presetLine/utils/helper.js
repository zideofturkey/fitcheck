...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for presetLine — no-op
};
 ...
 * on miss" pattern, use getCachedPresetLineById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for presetLine
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new PresetLineQueryCacheInvalidator();
