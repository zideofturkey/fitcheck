...
const updatePresetLineById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await PresetLine.findOne({ where });
    ...
    const updateResult = await PresetLine.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await PresetLine.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updatePresetLineById;
