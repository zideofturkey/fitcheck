...
// This script is called from the DeletePresetLineManager

const { PresetLine } = require("models");

const { PresetLineQueryCacheInvalidator } = require("./query-cache-classes");
...
const dbScriptDeletePresetline = async (apiManager) => {
  ...
  try {
    const updateResult = await PresetLine.update(
    ...
    if (!dbData && rowsCount > 0) {
      dbDoc = await PresetLine.findOne({ where: instanceWhereClause });
    ...
    const queryCacheInvalidator = new PresetLineQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeletePresetline",
      {
...
module.exports = dbScriptDeletePresetline;
