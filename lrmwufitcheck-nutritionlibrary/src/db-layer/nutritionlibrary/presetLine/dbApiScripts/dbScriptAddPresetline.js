...
// This script is called from the AddPresetLineManager
...
async function updateIfExists(dataClause) {
  const dbDoc = await PresetLine.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptAddPresetline = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await PresetLine.create(dataClause);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptAddPresetline",
      {
        whereClause: null,
...
module.exports = dbScriptAddPresetline;
