...
// This script is called from the _fetchListPresetLineManager

const { PresetLine, sequelize } = require("models");
...
const dbScript_fetchListpresetline = async (apiManager) => {
  ...
  try {
    ...
    let presetLineList = await PresetLine.findAll(options);

    const dbData = { items: [] };
    apiManager.presetLines = [];
    if (!presetLineList || presetLineList.length == 0) return dbData;

    dbData.items = Array.isArray(presetLineList)
      ? presetLineList
      : [presetLineList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("presetLine".id))'), "_COUNT"],
      ];
      let presetLine_count = await PresetLine.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      presetLine_count =
        presetLine_count && presetLine_count.length
          ? presetLine_count[0]._COUNT
          : 0;
      presetLine_count = isNaN(presetLine_count)
        ? presetLine_count
        : parseInt(presetLine_count);

      presetLineList = {
        rows: presetLineList,
        count: presetLine_count,
      };
      dbData.totalRowCount = presetLine_count;
    ...
    apiManager.presetLines = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListpresetline",
      {
...
module.exports = dbScript_fetchListpresetline;
