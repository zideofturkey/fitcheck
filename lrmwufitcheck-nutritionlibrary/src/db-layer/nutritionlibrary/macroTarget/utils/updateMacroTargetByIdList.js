...
const updateMacroTargetByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const macroTargetIdList = rows.map((item) => item.id);
    return macroTargetIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingMacroTargetByIdList",
      err,
    );
  }
};

module.exports = updateMacroTargetByIdList;
