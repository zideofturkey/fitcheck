...
const updateMacroTargetById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MacroTarget.findOne({ where });
    ...
    const updateResult = await MacroTarget.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await MacroTarget.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateMacroTargetById;
