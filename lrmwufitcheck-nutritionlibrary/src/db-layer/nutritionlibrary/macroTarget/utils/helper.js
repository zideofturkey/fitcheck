...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for macroTarget — no-op
};
 ...
 * on miss" pattern, use getCachedMacroTargetById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for macroTarget
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new MacroTargetQueryCacheInvalidator();
