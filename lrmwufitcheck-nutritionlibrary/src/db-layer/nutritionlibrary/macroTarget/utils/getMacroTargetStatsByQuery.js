...
/**
 * @deprecated Use getMacroTargetStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getMacroTargetStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(MacroTarget.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            MacroTarget.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            MacroTarget.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            MacroTarget.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            MacroTarget.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await MacroTarget.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingMacroTargetStatsByQuery",
      err,
    );
  }
};

module.exports = getMacroTargetStatsByQuery;
