...
// This script is called from the _fetchListMacroTargetManager

const { MacroTarget, sequelize } = require("models");
...
const dbScript_fetchListmacrotarget = async (apiManager) => {
  ...
  try {
    ...
    let macroTargetList = await MacroTarget.findAll(options);

    const dbData = { items: [] };
    apiManager.macroTargets = [];
    if (!macroTargetList || macroTargetList.length == 0) return dbData;

    dbData.items = Array.isArray(macroTargetList)
      ? macroTargetList
      : [macroTargetList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("macroTarget".id))'), "_COUNT"],
      ];
      let macroTarget_count = await MacroTarget.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      macroTarget_count =
        macroTarget_count && macroTarget_count.length
          ? macroTarget_count[0]._COUNT
          : 0;
      macroTarget_count = isNaN(macroTarget_count)
        ? macroTarget_count
        : parseInt(macroTarget_count);

      macroTargetList = {
        rows: macroTargetList,
        count: macroTarget_count,
      };
      dbData.totalRowCount = macroTarget_count;
    ...
    apiManager.macroTargets = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListmacrotarget",
      {
...
module.exports = dbScript_fetchListmacrotarget;
