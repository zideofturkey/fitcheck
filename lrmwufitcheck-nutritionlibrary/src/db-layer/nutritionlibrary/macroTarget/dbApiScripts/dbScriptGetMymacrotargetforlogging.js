...
// This script is called from the GetMyMacroTargetForLoggingManager
...
const dbScriptGetMymacrotargetforlogging = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await MacroTarget.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.macroTarget = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetMymacrotargetforlogging",
      {
...
module.exports = dbScriptGetMymacrotargetforlogging;
