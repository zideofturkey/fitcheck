...
// This script is called from the CreatePresetMealManager
...
async function updateIfExists(dataClause) {
  const dbDoc = await PresetMeal.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptCreatePresetmeal = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await PresetMeal.create(dataClause);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCreatePresetmeal",
      {
        whereClause: null,
...
module.exports = dbScriptCreatePresetmeal;
