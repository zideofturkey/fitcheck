...
// This script is called from the UpdatePresetMealManager

const { PresetMeal } = require("models");

const { PresetMealQueryCacheInvalidator } = require("./query-cache-classes");
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.presetMeal;
  return {
    old_presetMeal: oldData,
    presetMeal: newData,
...
const dbScriptUpdatePresetmeal = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdatePresetmeal] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdatePresetmeal] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdatePresetmeal] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdatePresetmeal] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await PresetMeal.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdatePresetmeal] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await PresetMeal.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdatePresetmeal] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    const queryCacheInvalidator = new PresetMealQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdatePresetmeal",
      {
...
module.exports = dbScriptUpdatePresetmeal;
