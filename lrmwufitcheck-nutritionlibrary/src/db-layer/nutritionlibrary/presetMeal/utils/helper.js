...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for presetMeal — no-op
};
 ...
 * on miss" pattern, use getCachedPresetMealById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for presetMeal
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new PresetMealQueryCacheInvalidator();
