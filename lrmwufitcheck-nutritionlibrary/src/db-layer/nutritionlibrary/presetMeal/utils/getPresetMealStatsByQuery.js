...
/**
 * @deprecated Use getPresetMealStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getPresetMealStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(PresetMeal.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            PresetMeal.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            PresetMeal.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            PresetMeal.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            PresetMeal.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await PresetMeal.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingPresetMealStatsByQuery",
      err,
    );
  }
};

module.exports = getPresetMealStatsByQuery;
