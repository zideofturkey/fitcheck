...
/**
 * @deprecated Use getFoodItemStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getFoodItemStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingFoodItemStatsByQuery",
      err,
    );
  }
};

module.exports = getFoodItemStatsByQuery;
