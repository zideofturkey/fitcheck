...
const updateFoodItemById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await FoodItem.findOne({ where });
    ...
    const updateResult = await FoodItem.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await FoodItem.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateFoodItemById;
