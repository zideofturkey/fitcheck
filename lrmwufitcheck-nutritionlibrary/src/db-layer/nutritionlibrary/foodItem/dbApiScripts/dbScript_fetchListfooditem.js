...
// This script is called from the _fetchListFoodItemManager
...
const dbScript_fetchListfooditem = async (apiManager) => {
  ...
  try {
    ...
    let foodItemList = await FoodItem.findAll(options);

    const dbData = { items: [] };
    apiManager.foodItems = [];
    if (!foodItemList || foodItemList.length == 0) return dbData;

    dbData.items = Array.isArray(foodItemList) ? foodItemList : [foodItemList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("foodItem".id))'), "_COUNT"],
      ];
      let foodItem_count = await FoodItem.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      foodItem_count =
        foodItem_count && foodItem_count.length ? foodItem_count[0]._COUNT : 0;
      foodItem_count = isNaN(foodItem_count)
        ? foodItem_count
        : parseInt(foodItem_count);

      foodItemList = {
        rows: foodItemList,
        count: foodItem_count,
      };
      dbData.totalRowCount = foodItem_count;
    ...
    apiManager.foodItems = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListfooditem",
      {
...
module.exports = dbScript_fetchListfooditem;
