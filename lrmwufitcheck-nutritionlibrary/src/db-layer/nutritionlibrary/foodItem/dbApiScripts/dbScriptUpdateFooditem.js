...
// This script is called from the UpdateFoodItemManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.foodItem;
  return {
...
const dbScriptUpdateFooditem = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateFooditem] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateFooditem] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdateFooditem] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateFooditem] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await FoodItem.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateFooditem] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await FoodItem.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateFooditem] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateFooditem",
      {
...
module.exports = dbScriptUpdateFooditem;
