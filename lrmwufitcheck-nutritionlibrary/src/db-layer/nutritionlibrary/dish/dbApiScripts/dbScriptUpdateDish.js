...
// This script is called from the UpdateDishManager

const { Dish } = require("models");

const { DishQueryCacheInvalidator } = require("./query-cache-classes");
...
const dbScriptUpdateDish = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateDish] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateDish] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdateDish] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateDish] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await Dish.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateDish] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await Dish.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateDish] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    const queryCacheInvalidator = new DishQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateDish",
      {
...
module.exports = dbScriptUpdateDish;
