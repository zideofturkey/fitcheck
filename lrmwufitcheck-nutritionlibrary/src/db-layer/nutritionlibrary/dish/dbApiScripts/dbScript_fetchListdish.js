...
// This script is called from the _fetchListDishManager
...
const dbScript_fetchListdish = async (apiManager) => {
  ...
  try {
    ...
    let dishList = await Dish.findAll(options);

    const dbData = { items: [] };
    apiManager.dishs = [];
    if (!dishList || dishList.length == 0) return dbData;

    dbData.items = Array.isArray(dishList) ? dishList : [dishList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("dish".id))'), "_COUNT"],
      ];
      let dish_count = await Dish.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      dish_count =
        dish_count && dish_count.length ? dish_count[0]._COUNT : 0;
      dish_count = isNaN(dish_count)
        ? dish_count
        : parseInt(dish_count);
      ...
      dbData.totalRowCount = dish_count;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListdish",
      {
...
module.exports = dbScript_fetchListdish;
