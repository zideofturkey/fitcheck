...
const createBulkDish = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await Dish.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await Dish.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenCreatingBulkDish", err);
  }
};

module.exports = createBulkDish;
