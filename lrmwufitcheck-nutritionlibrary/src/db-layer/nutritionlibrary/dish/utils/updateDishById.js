...
const updateDishById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Dish.findOne({ where });
    ...
    const updateResult = await Dish.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await Dish.findOne({ where: options.where });
    }
    if (!dbDoc) {
