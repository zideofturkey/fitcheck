...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for dish — no-op
};
 ...
 * on miss" pattern, use getCachedDishById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for dish
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new DishQueryCacheInvalidator();
