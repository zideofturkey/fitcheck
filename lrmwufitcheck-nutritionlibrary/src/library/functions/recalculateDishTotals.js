const { getDishLineListByMQuery, updateDishById } = require("dbLayer");

module.exports = async function recalculateDishTotals(dishId) {
  const lines = await getDishLineListByMQuery({
    dishId,
    isActive: true,
  });
  const totals = {
  ...
  if (lines && lines.length > 0) {
    for (const line of lines) {
      ...
      totals.totalGramWeight += line.gramAmount || 0;
    }
  }
  ...
  totals.totalGramWeight = Math.round(totals.totalGramWeight * 100) / 100;
  await updateDishById(dishId, totals);
};
