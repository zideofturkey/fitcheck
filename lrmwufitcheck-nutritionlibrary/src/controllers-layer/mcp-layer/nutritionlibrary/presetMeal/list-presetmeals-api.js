...
class ListPresetMealsMcpController extends NutritionLibraryServiceMcpController {
  constructor(params) {
    super("listPresetMeals", "listpresetmeals", params);
    this.dataName = "presetMeals";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListPresetMealsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: ListPresetMealsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListPresetMealsMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListPresetMealsMcpController.getOutputSchema().parse(result);
