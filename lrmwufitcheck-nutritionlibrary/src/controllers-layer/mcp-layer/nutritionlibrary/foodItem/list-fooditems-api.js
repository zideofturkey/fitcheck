...
class ListFoodItemsMcpController extends NutritionLibraryServiceMcpController {
  constructor(params) {
    super("listFoodItems", "listfooditems", params);
    this.dataName = "foodItems";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListFoodItemsManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      searchTerm: z
        ...
        .max(255)
        .optional()
        .describe("Optional partial match on foodName"),

      foodCategory: z.string().max(255).optional().describe(""),

      creationSource: z
        .enum(["manualEntry", "aiAssistant"])
        .optional()
        .describe(""),
    };
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: ListFoodItemsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListFoodItemsMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListFoodItemsMcpController.getOutputSchema().parse(result);
