...
class UpdateDishMcpController extends NutritionLibraryServiceMcpController {
  constructor(params) {
    super("updateDish", "updatedish", params);
    this.dataName = "dish";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateDishManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      dishName: z.string().max(255).optional().describe(""),
...
module.exports = (headers) => {
  ...
  return {
    name: "updateDish",
    description:
      "Update dish header fields (dishName, descriptionText). Nutrition totals are NOT updated here." +
      requiredRolesMarker,
    requiredRoles,
    parameters: UpdateDishMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new UpdateDishMcpController(mcpParams);
      try {
