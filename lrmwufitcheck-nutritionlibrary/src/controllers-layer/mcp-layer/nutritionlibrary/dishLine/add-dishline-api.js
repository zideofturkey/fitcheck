...
class AddDishLineMcpController extends NutritionLibraryServiceMcpController {
  constructor(params) {
    super("addDishLine", "adddishline", params);
    this.dataName = "dishLine";
    this.crudType = "create";
  }

  createApiManager() {
    return new AddDishLineManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    name: "addDishLine",
    description:
      "Add a food item line to a dish. Validates dish ownership and food item ownership, calculates nutrition snapshot, creates the line, then recalculates parent dish totals." +
      requiredRolesMarker,
    requiredRoles,
    parameters: AddDishLineMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new AddDishLineMcpController(mcpParams);
      try {
