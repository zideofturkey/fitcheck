...
class _fetchListPresetLineMcpController extends NutritionLibraryServiceMcpController {
  constructor(params) {
    super("_fetchListPresetLine", "_fetchlistpresetline", params);
    this.dataName = "presetLines";
    this.crudType = "list";
  }

  createApiManager() {
    return new _fetchListPresetLineManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      presetMealId: z.string().uuid().optional().describe(""),
    };
  }
}
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: _fetchListPresetLineMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new _fetchListPresetLineMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return _fetchListPresetLineMcpController.getOutputSchema().parse(result);
