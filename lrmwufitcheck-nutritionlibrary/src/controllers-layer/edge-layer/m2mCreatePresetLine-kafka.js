...
const { m2mCreatePresetLine } = require("edgeFunctions");

const m2mCreatePresetLineHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mCreatePresetLineInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mCreatePresetLineRequiresM2MToken",
    ...
    const result = await m2mCreatePresetLine(message);
    // log result
  } catch (err) {
    console.error("Error running routeService for m2mCreatePresetLine: ", err);
    // log error
  }
};

module.exports = m2mCreatePresetLineHandler;
