...
const { m2mUpdatePresetMealByQuery } = require("edgeFunctions");
const createServiceController = require("restLayer/create-service-controller");

const m2mUpdatePresetMealByQueryRestController = async (req, res, next) => {
  try {
    req.loginRequired = false;
    const statusCode = 200;

    const restController = createServiceController(
      "m2mUpdatePresetMealByQuery",
      "m2mUpdatePresetMealByQuery",
      req,
      res,
    );
    ...
    if (req.M2MToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdatePresetMealByQueryInvalidM2MToken",
    ...
    const result = await m2mUpdatePresetMealByQuery(req);
    ...
    console.error(
      "Error running routeService for m2mUpdatePresetMealByQuery: ",
      err,
    );
    return next(err);
  }
...
module.exports = m2mUpdatePresetMealByQueryRestController;
