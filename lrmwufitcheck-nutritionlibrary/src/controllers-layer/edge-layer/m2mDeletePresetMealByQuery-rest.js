...
const { m2mDeletePresetMealByQuery } = require("edgeFunctions");
const createServiceController = require("restLayer/create-service-controller");

const m2mDeletePresetMealByQueryRestController = async (req, res, next) => {
  try {
    req.loginRequired = false;
    const statusCode = 200;

    const restController = createServiceController(
      "m2mDeletePresetMealByQuery",
      "m2mDeletePresetMealByQuery",
      req,
      res,
    );
    ...
    if (req.M2MToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mDeletePresetMealByQueryInvalidM2MToken",
    ...
    const result = await m2mDeletePresetMealByQuery(req);
    ...
    console.error(
      "Error running routeService for m2mDeletePresetMealByQuery: ",
      err,
    );
    return next(err);
  }
...
module.exports = m2mDeletePresetMealByQueryRestController;
