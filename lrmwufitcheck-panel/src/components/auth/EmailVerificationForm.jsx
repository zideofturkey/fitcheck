...
export default function EmailVerificationForm({ onComplete, adminMode }) {
    ...
    if (result.success) {
      ...
      if (result.errCode === "AlreadyVerified") {
        if (adminMode) {
          ...
          setError("Email is already verified. You can now login.");
        }
      } else {
  ...
  const handleComplete = async (e) => {
    ...
    setError(null);
    const result = await completeEmailVerification(email, code.trim());

    if (result.success) {
      if (adminMode) {
        // In admin mode, show success and call onComplete callback
        setSuccess(true);
        setTimeout(() => {
