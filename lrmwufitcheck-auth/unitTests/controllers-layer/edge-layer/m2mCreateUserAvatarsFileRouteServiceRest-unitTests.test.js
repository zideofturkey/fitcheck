...
describe("m2mCreateUserAvatarsFileRestController", function () {
  let functionNameStub;
  let m2mCreateUserAvatarsFileRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateUserAvatarsFileRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateUserAvatarsFile-rest",
      {
        edgeFunctions: { m2mCreateUserAvatarsFile: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateUserAvatarsFileRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateUserAvatarsFileRestController = (req, res, next) => {
      next(error);
    };
