...
describe("m2mBulkCreateUserRestController", function () {
  let functionNameStub;
  let m2mBulkCreateUserRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateUserRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateUser-rest",
      {
        edgeFunctions: { m2mBulkCreateUser: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateUserRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateUserRestController = (req, res, next) => {
      next(error);
    };
