...
describe("m2mUpdateUserByIdRestController", function () {
  let functionNameStub;
  let m2mUpdateUserByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateUserByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateUserById-rest",
      {
        edgeFunctions: { m2mUpdateUserById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateUserByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateUserByIdRestController = (req, res, next) => {
      next(error);
    };
