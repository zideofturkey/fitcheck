...
describe("m2mUpdateUserByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateUserByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateUserByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateUserByQuery-rest",
      {
        edgeFunctions: { m2mUpdateUserByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateUserByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateUserByQueryRestController = (req, res, next) => {
      next(error);
    };
