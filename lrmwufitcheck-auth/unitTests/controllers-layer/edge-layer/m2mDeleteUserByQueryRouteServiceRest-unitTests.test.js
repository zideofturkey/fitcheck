...
describe("m2mDeleteUserByQueryRestController", function () {
  let functionNameStub;
  let m2mDeleteUserByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteUserByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteUserByQuery-rest",
      {
        edgeFunctions: { m2mDeleteUserByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteUserByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteUserByQueryRestController = (req, res, next) => {
      next(error);
    };
