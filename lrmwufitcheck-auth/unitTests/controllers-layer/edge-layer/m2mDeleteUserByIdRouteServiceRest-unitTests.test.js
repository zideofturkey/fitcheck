...
describe("m2mDeleteUserByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteUserByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteUserByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteUserById-rest",
      {
        edgeFunctions: { m2mDeleteUserById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteUserByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteUserByIdRestController = (req, res, next) => {
      next(error);
    };
