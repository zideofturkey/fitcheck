...
describe("m2mUpdateUserByIdListRestController", function () {
  let functionNameStub;
  let m2mUpdateUserByIdListRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateUserByIdListRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateUserByIdList-rest",
      {
        edgeFunctions: { m2mUpdateUserByIdList: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateUserByIdListRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateUserByIdListRestController = (req, res, next) => {
      next(error);
    };
