...
describe("m2mCreateUserRestController", function () {
  let functionNameStub;
  let m2mCreateUserRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateUserRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateUser-rest",
      {
        edgeFunctions: { m2mCreateUser: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateUserRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateUserRestController = (req, res, next) => {
      next(error);
    };
