...
- **Raise Api Event** : `true`
  ...
  The event will be emitted to the `userpasswordbyadmin-updated` Kafka Topic Note that the DB-Level events for `create`, `update` and `delete` operations will always be raised for internal reasons.
...
```js
{
  password: runMScript(() => (this.password), {"path":"services[0].businessLogic[10].dataClauseItems[0].value"}),
}
```
