...
class UpdateUserRoleManager extends UserManager {
  ...
  async buildDataClause() {
    ...
    const dataClause = {
      roleId: runMScript(() => this.roleId, {
        path: "services[0].businessLogic[8].dataClauseItems[0].value",
      }),
    };
  ...
  async executeMainOperation() {
    return await dbScriptUpdateUserrole(this);
  ...
  async afterCheckBasicAuth() {
    ...
    try {
      await this.preventHigherRoleSet();
    } catch (err) {
      console.log("preventHigherRoleSet Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  /***********************************************************************
   ** Prevents to set a user's role as higher than or equal to the setter
   ** role
   ***********************************************************************/
