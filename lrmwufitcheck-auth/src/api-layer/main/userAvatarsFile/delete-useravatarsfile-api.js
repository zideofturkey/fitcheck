...
const { dbScriptDeleteUseravatarsfile } = require("dbLayer");
...
class DeleteUserAvatarsFileManager extends UserAvatarsFileManager {
  constructor(request, controllerType) {
    super(request, {
      name: "deleteUserAvatarsFile",
  ...
  async getRouteQuery() {
    return runMScript(() => ({ id: this.userAvatarsFileId }), {
      path: "services[0].businessLogic[15].whereClause.fullWhereClause",
    });
  ...
  async fetchInstance() {
    const { getUserAvatarsFileByQuery } = require("dbLayer");

    console.log("this.whereClause -->", this.whereClause);
    this.userAvatarsFile = await getUserAvatarsFileByQuery(this.whereClause);
    if (!this.userAvatarsFile) {
      throw new NotFoundError("errMsg_RecordNotFound");
    }
    this._instance = this.userAvatarsFile;
    this.instance = this.userAvatarsFile;
  }
  ...
  async executeMainOperation() {
    return await dbScriptDeleteUseravatarsfile(this);
  ...
  async raiseEvent() {
    UseravatarsfileDeletedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = DeleteUserAvatarsFileManager;
