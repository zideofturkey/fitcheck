...
console.log("Starting fitcheck-auth-service service...");
...
const start = async () => {
  process.title = "node-lrmwufitcheck-auth-service";
  ...
  try {
    ...
    await setRedisData("lrmwufitcheck-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key lrmwufitcheck-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  // Init login session and load RSA key ID before the server starts accepting requests.
  // setCurrentKeyId must run before listen() — any login attempt before it runs sees
  // global.currentKeyId === undefined, which makes the key path "rsa.key.undefined".
  ...
  console.log(
    "fitcheck-auth-service is listening HTTP/REST port " +
...
/* starting the server fitcheck-auth-service .... */
start();
