...
/**
 * Calculates statistics on User records matching an MScript Query.
 ...
 */
const getUserStatsByMQuery = async (mQuery, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            User.aggregate(statParts[1], "avg", { where: queryWithSoftDelete }),
          );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingUserStatsByMQuery",
      err,
    );
  }
};

module.exports = getUserStatsByMQuery;
