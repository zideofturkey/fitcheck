...
const { UserEntityCache } = require("../dbApiScripts/entity-cache-classes");
const {
  UserQueryCacheInvalidator,
} = require("../dbApiScripts/query-cache-classes");

const objectName = "user";
const serviceCodename = "lrmwufitcheck-auth-service";
...
const _saveEntityToCache = async (data) => {
  ...
  try {
    ...
    entityCacher.defaultId = data.id;
    await entityCacher.saveEntityToCache(data);
  } catch (err) {
    //**errorLog
    console.error(`EntityCache write failed for ${objectName}:`, err.message);
  }
};
 ...
 * on miss" pattern, use getCachedUserById instead.
...
const getEntityFromCache = async (id) => {
  ...
  try {
    const entityCacher = new UserEntityCache();
    return await entityCacher.getEntityFromCache(id);
  } catch (err) {
    //**errorLog
    console.error(`EntityCache read failed for ${objectName}:`, err.message);
    return null;
  }
};
...
const deleteEntityCache = async (id) => {
  ...
  try {
    const entityCacher = new UserEntityCache();
    await entityCacher.delEntityFromCache(id);
  } catch (err) {
    //**errorLog
    console.error(`EntityCache delete failed for ${objectName}:`, err.message);
  }
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new UserQueryCacheInvalidator();
