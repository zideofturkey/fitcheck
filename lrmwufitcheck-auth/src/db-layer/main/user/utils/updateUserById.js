...
const updateUserById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await User.findOne({ where });
    ...
    const updateResult = await User.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await User.findOne({ where: options.where });
    }
    if (!dbDoc) {
