...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: email-index (throwError)
  if (remaining.length > 0) {
    const getKey_email_index = (r) => String(r.email ?? null);

    const orConditions_email_index = remaining.map((data) => ({
      email: data.email ?? null,
    }));

    const conflicts_email_index = await User.findAll({
      where: { [Op.or]: orConditions_email_index, isActive: true },
    });

    if (conflicts_email_index.length > 0) {
      const conflictMap_email_index = new Map();
      conflicts_email_index.forEach((dbDoc) => {
        conflictMap_email_index.set(getKey_email_index(dbDoc), dbDoc);
      });

      // throwError: abort entire bulk operation if any record conflicts
      const matched_email_index = remaining.filter((d) =>
        conflictMap_email_index.has(getKey_email_index(d)),
      );
      if (matched_email_index.length > 0) {
        throw new BadRequestError(
          "errMsg_DuplicateUniqueIndexError:email-index",
        );
      }
    }
  }
...
const createBulkUser = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await User.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await User.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenCreatingBulkUser", err);
  }
};

module.exports = createBulkUser;
