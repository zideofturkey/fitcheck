...
// This script is called from the UpdateUserPasswordByAdminManager
...
const dbScriptUpdateUserpasswordbyadmin = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateUserpasswordbyadmin] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateUserpasswordbyadmin] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdateUserpasswordbyadmin] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateUserpasswordbyadmin] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await User.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateUserpasswordbyadmin] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await User.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateUserpasswordbyadmin] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateUserpasswordbyadmin",
      {
...
module.exports = dbScriptUpdateUserpasswordbyadmin;
