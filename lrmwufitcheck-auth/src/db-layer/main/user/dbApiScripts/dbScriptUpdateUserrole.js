...
// This script is called from the UpdateUserRoleManager
...
const dbScriptUpdateUserrole = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateUserrole] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateUserrole] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptUpdateUserrole] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateUserrole] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateUserrole] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await User.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateUserrole] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateUserrole",
      {
...
module.exports = dbScriptUpdateUserrole;
