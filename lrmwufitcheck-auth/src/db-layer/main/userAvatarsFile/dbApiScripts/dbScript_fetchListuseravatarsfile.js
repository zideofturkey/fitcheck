...
// This script is called from the _fetchListUserAvatarsFileManager

const { UserAvatarsFile, sequelize } = require("models");
...
const dbScript_fetchListuseravatarsfile = async (apiManager) => {
  ...
  try {
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("userAvatarsFile".id))'), "_COUNT"],
      ];
      let userAvatarsFile_count = await UserAvatarsFile.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      userAvatarsFile_count =
        userAvatarsFile_count && userAvatarsFile_count.length
          ? userAvatarsFile_count[0]._COUNT
          : 0;
      userAvatarsFile_count = isNaN(userAvatarsFile_count)
        ? userAvatarsFile_count
        : parseInt(userAvatarsFile_count);

      userAvatarsFileList = {
        rows: userAvatarsFileList,
        count: userAvatarsFile_count,
      };
      dbData.totalRowCount = userAvatarsFile_count;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListuseravatarsfile",
      {
...
module.exports = dbScript_fetchListuseravatarsfile;
