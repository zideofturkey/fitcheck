...
const updateUserAvatarsFileById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await UserAvatarsFile.findOne({ where: { id } });
    ...
    const updateResult = await UserAvatarsFile.update(
      resolvedDataClause,
      options,
    );
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await UserAvatarsFile.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateUserAvatarsFileById;
