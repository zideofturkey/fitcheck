...
const objectName = "userAvatarsFile";
...
const raiseDbEventUpdate = async (
  ...
  const eventData = {
    old_userAvatarsFile: oldData,
    userAvatarsFile: newData,
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for userAvatarsFile — no-op
};
 ...
 * on miss" pattern, use getCachedUserAvatarsFileById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for userAvatarsFile
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new UserAvatarsFileQueryCacheInvalidator();
