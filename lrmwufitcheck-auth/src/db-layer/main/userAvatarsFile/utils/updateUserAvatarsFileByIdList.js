...
const updateUserAvatarsFileByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const userAvatarsFileIdList = rows.map((item) => item.id);
    return userAvatarsFileIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingUserAvatarsFileByIdList",
      err,
    );
  }
};

module.exports = updateUserAvatarsFileByIdList;
