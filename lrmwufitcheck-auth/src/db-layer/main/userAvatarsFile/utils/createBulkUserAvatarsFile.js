...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: accessKey-index (throwError)
  if (remaining.length > 0) {
    const getKey_accessKey_index = (r) => String(r.accessKey ?? null);

    const orConditions_accessKey_index = remaining.map((data) => ({
      accessKey: data.accessKey ?? null,
    }));

    const conflicts_accessKey_index = await UserAvatarsFile.findAll({
      where: { [Op.or]: orConditions_accessKey_index },
    });

    if (conflicts_accessKey_index.length > 0) {
      const conflictMap_accessKey_index = new Map();
      conflicts_accessKey_index.forEach((dbDoc) => {
        conflictMap_accessKey_index.set(getKey_accessKey_index(dbDoc), dbDoc);
      });

      // throwError: abort entire bulk operation if any record conflicts
      const matched_accessKey_index = remaining.filter((d) =>
        conflictMap_accessKey_index.has(getKey_accessKey_index(d)),
      );
      if (matched_accessKey_index.length > 0) {
...
const createBulkUserAvatarsFile = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await UserAvatarsFile.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await UserAvatarsFile.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkUserAvatarsFile",
      err,
    );
  }
};

module.exports = createBulkUserAvatarsFile;
