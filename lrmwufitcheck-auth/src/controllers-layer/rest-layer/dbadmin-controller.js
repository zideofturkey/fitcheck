...
// Login-session class — used by the reset endpoint to re-seed the
// superAdmin row (and any other auth-side bootstrap data) after a
// destructive sync drops the user table.
const FitcheckLoginSession = require("../../project-session/fitcheck-login-session");
...
const MODEL_REGISTRY = {
  user: {
    modelName: "User",
    objectName: "user",
    schemaPath: "../../models/schemas/user.schema",
    getModel: () => require("../../models").User,
    getTableName: () => {
      try {
        const Model = require("../../models").User;
        return Model?.getTableName?.() || "user";
      } catch {
        return "user";
      }
    },
  ...
  userAvatarsFile: {
    ...
    schemaPath: "../../models/schemas/userAvatarsFile.schema",
    getModel: () => require("../../models").UserAvatarsFile,
    getTableName: () => {
      try {
        const Model = require("../../models").UserAvatarsFile;
        return Model?.getTableName?.() || "userAvatarsFile";
      } catch {
        return "userAvatarsFile";
      }
    },
  },
};
...
router.post("/v1/_dbadmin/database/reset", async (req, res) => {
  ...
  try {
    ...
    // Re-seed the auth-side bootstrap rows (superAdmin, admin group,
    // root tenant if multi-tenant, etc.) the same way startup does it.
    // After sync({ force: true }) every table is empty, including the
    // user table — without this re-seed loginAsSuperadmin would fail
    // until the next service restart and any tool that holds an
    // already-issued JWT for the superAdmin would suddenly point at a
    // missing row. Re-running init() puts those rows back with the
    // SAME ids that startup originally used (the seed function reads
    // ids from the LoginSession class, which are deterministic per
    // project), so existing tokens / sessions remain coherent.
    //
    // This MUST run after the ES + Redis cache wipes above — the seed
    // writes superAdmin via the managed CRUD path, which triggers
    // indexDataToElastic and cache writes. Those need clean state to
    // land in, otherwise the seed row's ES doc / cache entries collide
    // with whatever the previous run left behind.
    let reseed = "ok";
    try {
      const loginSession = new FitcheckLoginSession();
    ...
    res.json({
      ...
      message:
        ...
        (reseed === "ok"
          ? " Auth bootstrap rows re-seeded."
          : ` WARNING: auth re-seed failed (${reseed}); restart the service to recover.`),
    });
  } catch (err) {
    res.status(500).json({
      status: "failed",
