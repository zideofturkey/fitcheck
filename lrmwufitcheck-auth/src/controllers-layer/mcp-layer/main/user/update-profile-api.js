...
class UpdateProfileMcpController extends AuthServiceMcpController {
  constructor(params) {
    super("updateProfile", "updateprofile", params);
    this.dataName = "user";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateProfileManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      fullname: z.string().max(255).optional().describe("User's full name."),

      avatar: z.string().max(255).optional().describe("Avatar URL."),
    };
  }
}
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: UpdateProfileMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new UpdateProfileMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return UpdateProfileMcpController.getOutputSchema().parse(result);
