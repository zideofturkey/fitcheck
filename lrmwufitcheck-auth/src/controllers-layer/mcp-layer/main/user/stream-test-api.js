...
class StreamTestMcpController extends AuthServiceMcpController {
  constructor(params) {
    super("streamTest", "streamtest", params);
    this.dataName = "user";
    this.crudType = "get";
  }

  createApiManager() {
    return new StreamTestManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    name: "streamTest",
    description:
      "Test API for iterator action streaming via SSE." + requiredRolesMarker,
    requiredRoles,
    parameters: StreamTestMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new StreamTestMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return StreamTestMcpController.getOutputSchema().parse(result);
