...
class GetUserMcpController extends AuthServiceMcpController {
  constructor(params) {
    super("getUser", "getuser", params);
    this.dataName = "user";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetUserManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: GetUserMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetUserMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetUserMcpController.getOutputSchema().parse(result);
