...
class UpdateUserMcpController extends AuthServiceMcpController {
  constructor(params) {
    super("updateUser", "updateuser", params);
    this.dataName = "user";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateUserManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: UpdateUserMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new UpdateUserMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return UpdateUserMcpController.getOutputSchema().parse(result);
