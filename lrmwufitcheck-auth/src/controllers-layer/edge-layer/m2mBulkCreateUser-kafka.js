...
const { m2mBulkCreateUser } = require("edgeFunctions");

const m2mBulkCreateUserHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mBulkCreateUserInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mBulkCreateUserRequiresM2MToken",
    ...
    const result = await m2mBulkCreateUser(message);
    // log result
  } catch (err) {
    console.error("Error running routeService for m2mBulkCreateUser: ", err);
    // log error
  }
};

module.exports = m2mBulkCreateUserHandler;
