...
const { m2mCreateUser } = require("edgeFunctions");

const m2mCreateUserHandler = async (topic, session, message, callbackData) => {
  ...
  try {
    ...
    const result = await m2mCreateUser(message);
    // log result
  } catch (err) {
    console.error("Error running routeService for m2mCreateUser: ", err);
    // log error
  }
};

module.exports = m2mCreateUserHandler;
