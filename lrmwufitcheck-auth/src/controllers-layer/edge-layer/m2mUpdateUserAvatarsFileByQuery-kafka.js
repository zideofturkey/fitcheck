...
const { m2mUpdateUserAvatarsFileByQuery } = require("edgeFunctions");

const m2mUpdateUserAvatarsFileByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateUserAvatarsFileByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateUserAvatarsFileByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateUserAvatarsFileByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateUserAvatarsFileByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateUserAvatarsFileByQueryHandler;
