...
const { m2mDeleteUserAvatarsFileByQuery } = require("edgeFunctions");

const m2mDeleteUserAvatarsFileByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mDeleteUserAvatarsFileByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mDeleteUserAvatarsFileByQueryRequiresM2MToken",
    ...
    const result = await m2mDeleteUserAvatarsFileByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mDeleteUserAvatarsFileByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mDeleteUserAvatarsFileByQueryHandler;
