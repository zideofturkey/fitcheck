...
describe("m2mCreateSys_toolCatalogRestController", function () {
  let functionNameStub;
  let m2mCreateSys_toolCatalogRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateSys_toolCatalogRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateSys_toolCatalog-rest",
      {
        edgeFunctions: { m2mCreateSys_toolCatalog: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateSys_toolCatalogRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateSys_toolCatalogRestController = (req, res, next) => {
      next(error);
    };
