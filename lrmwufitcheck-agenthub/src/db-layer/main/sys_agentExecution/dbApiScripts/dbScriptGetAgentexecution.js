...
// This script is called from the GetAgentExecutionManager
...
const dbScriptGetAgentexecution = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await Sys_agentExecution.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.sys_agentExecution = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetAgentexecution",
      {
...
module.exports = dbScriptGetAgentexecution;
