...
// This script is called from the _fetchListSys_agentExecutionManager

const { Sys_agentExecution, sequelize } = require("models");
...
const dbScript_fetchListsys_agentexecution = async (apiManager) => {
  ...
  try {
    ...
    let sys_agentExecutionList = await Sys_agentExecution.findAll(options);

    const dbData = { items: [] };
    apiManager.sys_agentExecutions = [];
    if (!sys_agentExecutionList || sys_agentExecutionList.length == 0)
      return dbData;

    dbData.items = Array.isArray(sys_agentExecutionList)
      ? sys_agentExecutionList
      : [sys_agentExecutionList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [
          sequelize.literal('COUNT(DISTINCT("sys_agentExecution".id))'),
          "_COUNT",
        ],
      ];
      let sys_agentExecution_count = await Sys_agentExecution.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      sys_agentExecution_count =
        sys_agentExecution_count && sys_agentExecution_count.length
          ? sys_agentExecution_count[0]._COUNT
          : 0;
      sys_agentExecution_count = isNaN(sys_agentExecution_count)
        ? sys_agentExecution_count
        : parseInt(sys_agentExecution_count);

      sys_agentExecutionList = {
        rows: sys_agentExecutionList,
        count: sys_agentExecution_count,
      };
      dbData.totalRowCount = sys_agentExecution_count;
    ...
    apiManager.sys_agentExecutions = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListsys_agentexecution",
      {
...
module.exports = dbScript_fetchListsys_agentexecution;
