...
const createBulkSys_agentExecution = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await Sys_agentExecution.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await Sys_agentExecution.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkSys_agentExecution",
      err,
    );
  }
};

module.exports = createBulkSys_agentExecution;
