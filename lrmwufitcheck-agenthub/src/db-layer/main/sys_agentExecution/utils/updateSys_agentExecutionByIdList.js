...
const updateSys_agentExecutionByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const sys_agentExecutionIdList = rows.map((item) => item.id);
    return sys_agentExecutionIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingSys_agentExecutionByIdList",
      err,
    );
  }
};

module.exports = updateSys_agentExecutionByIdList;
