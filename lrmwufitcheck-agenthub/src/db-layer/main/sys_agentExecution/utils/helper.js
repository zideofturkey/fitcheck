...
const objectName = "sys_agentExecution";
...
const raiseDbEventUpdate = async (
  ...
  const eventData = {
    old_sys_agentExecution: oldData,
    sys_agentExecution: newData,
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for sys_agentExecution — no-op
};
 ...
 * on miss" pattern, use getCachedSys_agentExecutionById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for sys_agentExecution
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new Sys_agentExecutionQueryCacheInvalidator();
