...
const deleteSys_agentExecutionById = async (id, context = null) => {
  try {
    if (typeof id === "object") {
      id = id.id;
    }
    ...
    const existingDoc = await Sys_agentExecution.findByPk(id);
    if (!existingDoc) {
...
module.exports = deleteSys_agentExecutionById;
