...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: toolName-index (throwError)
  if (remaining.length > 0) {
    const getKey_toolName_index = (r) => String(r.toolName ?? null);

    const orConditions_toolName_index = remaining.map((data) => ({
      toolName: data.toolName ?? null,
    }));

    const conflicts_toolName_index = await Sys_toolCatalog.findAll({
      where: { [Op.or]: orConditions_toolName_index },
    });

    if (conflicts_toolName_index.length > 0) {
      const conflictMap_toolName_index = new Map();
      conflicts_toolName_index.forEach((dbDoc) => {
        conflictMap_toolName_index.set(getKey_toolName_index(dbDoc), dbDoc);
      });

      // throwError: abort entire bulk operation if any record conflicts
      const matched_toolName_index = remaining.filter((d) =>
        conflictMap_toolName_index.has(getKey_toolName_index(d)),
      );
      if (matched_toolName_index.length > 0) {
...
const createBulkSys_toolCatalog = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await Sys_toolCatalog.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await Sys_toolCatalog.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkSys_toolCatalog",
      err,
    );
  }
};

module.exports = createBulkSys_toolCatalog;
