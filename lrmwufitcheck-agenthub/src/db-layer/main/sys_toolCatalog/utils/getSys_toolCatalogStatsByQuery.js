...
/**
 * @deprecated Use getSys_toolCatalogStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getSys_toolCatalogStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Sys_toolCatalog.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            Sys_toolCatalog.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            Sys_toolCatalog.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            Sys_toolCatalog.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            Sys_toolCatalog.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await Sys_toolCatalog.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingSys_toolCatalogStatsByQuery",
      err,
    );
  }
};

module.exports = getSys_toolCatalogStatsByQuery;
