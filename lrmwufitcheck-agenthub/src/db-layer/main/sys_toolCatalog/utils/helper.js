...
const objectName = "sys_toolCatalog";
...
const raiseDbEventUpdate = async (
  ...
  const eventData = {
    old_sys_toolCatalog: oldData,
    sys_toolCatalog: newData,
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for sys_toolCatalog — no-op
};
 ...
 * on miss" pattern, use getCachedSys_toolCatalogById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for sys_toolCatalog
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new Sys_toolCatalogQueryCacheInvalidator();
