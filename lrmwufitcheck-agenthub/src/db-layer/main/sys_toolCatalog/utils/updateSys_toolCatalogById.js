...
const updateSys_toolCatalogById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Sys_toolCatalog.findOne({ where: { id } });
    ...
    const updateResult = await Sys_toolCatalog.update(
      resolvedDataClause,
      options,
    );
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await Sys_toolCatalog.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateSys_toolCatalogById;
