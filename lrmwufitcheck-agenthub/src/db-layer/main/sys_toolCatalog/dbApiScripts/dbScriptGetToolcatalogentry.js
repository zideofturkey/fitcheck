...
// This script is called from the GetToolCatalogEntryManager
...
const dbScriptGetToolcatalogentry = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await Sys_toolCatalog.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.sys_toolCatalog = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetToolcatalogentry",
      {
...
module.exports = dbScriptGetToolcatalogentry;
