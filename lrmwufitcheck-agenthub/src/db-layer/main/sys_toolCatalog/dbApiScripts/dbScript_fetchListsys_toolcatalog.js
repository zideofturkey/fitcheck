...
// This script is called from the _fetchListSys_toolCatalogManager

const { Sys_toolCatalog, sequelize } = require("models");
...
const dbScript_fetchListsys_toolcatalog = async (apiManager) => {
  ...
  try {
    ...
    let sys_toolCatalogList = await Sys_toolCatalog.findAll(options);

    const dbData = { items: [] };
    apiManager.sys_toolCatalogs = [];
    if (!sys_toolCatalogList || sys_toolCatalogList.length == 0) return dbData;

    dbData.items = Array.isArray(sys_toolCatalogList)
      ? sys_toolCatalogList
      : [sys_toolCatalogList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("sys_toolCatalog".id))'), "_COUNT"],
      ];
      let sys_toolCatalog_count = await Sys_toolCatalog.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      sys_toolCatalog_count =
        sys_toolCatalog_count && sys_toolCatalog_count.length
          ? sys_toolCatalog_count[0]._COUNT
          : 0;
      sys_toolCatalog_count = isNaN(sys_toolCatalog_count)
        ? sys_toolCatalog_count
        : parseInt(sys_toolCatalog_count);

      sys_toolCatalogList = {
        rows: sys_toolCatalogList,
        count: sys_toolCatalog_count,
      };
      dbData.totalRowCount = sys_toolCatalog_count;
    ...
    apiManager.sys_toolCatalogs = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListsys_toolcatalog",
      {
...
module.exports = dbScript_fetchListsys_toolcatalog;
