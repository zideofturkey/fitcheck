...
const deleteSys_agentConversationById = async (id, context = null) => {
  try {
    if (typeof id === "object") {
      id = id.id;
    }
    ...
    const existingDoc = await Sys_agentConversation.findByPk(id);
    if (!existingDoc) {
...
module.exports = deleteSys_agentConversationById;
