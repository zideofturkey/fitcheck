...
const updateSys_agentConversationByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const sys_agentConversationIdList = rows.map((item) => item.id);
    return sys_agentConversationIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingSys_agentConversationByIdList",
      err,
    );
  }
};

module.exports = updateSys_agentConversationByIdList;
