...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: sessionId-index (throwError)
  if (remaining.length > 0) {
    const getKey_sessionId_index = (r) => String(r.sessionId ?? null);

    const orConditions_sessionId_index = remaining.map((data) => ({
      sessionId: data.sessionId ?? null,
    }));

    const conflicts_sessionId_index = await Sys_agentConversation.findAll({
      where: { [Op.or]: orConditions_sessionId_index },
    });

    if (conflicts_sessionId_index.length > 0) {
      const conflictMap_sessionId_index = new Map();
      conflicts_sessionId_index.forEach((dbDoc) => {
        conflictMap_sessionId_index.set(getKey_sessionId_index(dbDoc), dbDoc);
      });

      // throwError: abort entire bulk operation if any record conflicts
      const matched_sessionId_index = remaining.filter((d) =>
        conflictMap_sessionId_index.has(getKey_sessionId_index(d)),
      );
      if (matched_sessionId_index.length > 0) {
...
const createBulkSys_agentConversation = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await Sys_agentConversation.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await Sys_agentConversation.bulkCreate(
        finalRecordsToCreate,
        {
          returning: true,
          validate: true,
        },
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkSys_agentConversation",
      err,
    );
  }
};

module.exports = createBulkSys_agentConversation;
