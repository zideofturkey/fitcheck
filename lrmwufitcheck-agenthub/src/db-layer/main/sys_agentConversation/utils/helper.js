...
const objectName = "sys_agentConversation";
const serviceCodename = "lrmwufitcheck-agenthub-service";
...
const raiseDbEventUpdate = async (
  ...
  const eventData = {
    old_sys_agentConversation: oldData,
    sys_agentConversation: newData,
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for sys_agentConversation — no-op
};
 ...
 * on miss" pattern, use getCachedSys_agentConversationById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for sys_agentConversation
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new Sys_agentConversationQueryCacheInvalidator();
