...
/**
 * @deprecated Use getSys_agentConversationStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getSys_agentConversationStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          Sys_agentConversation.count({ where: queryWithSoftDelete }),
        );
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            Sys_agentConversation.sum(statParts[1], {
          ...
          promises.push(
            Sys_agentConversation.aggregate(statParts[1], "avg", {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            Sys_agentConversation.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            Sys_agentConversation.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await Sys_agentConversation.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingSys_agentConversationStatsByQuery",
      err,
    );
  }
};

module.exports = getSys_agentConversationStatsByQuery;
