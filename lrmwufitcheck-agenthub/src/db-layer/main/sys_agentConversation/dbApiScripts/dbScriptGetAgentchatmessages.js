...
// This script is called from the GetAgentChatMessagesManager
...
const dbScriptGetAgentchatmessages = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await Sys_agentConversation.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.sys_agentConversation = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetAgentchatmessages",
      {
...
module.exports = dbScriptGetAgentchatmessages;
