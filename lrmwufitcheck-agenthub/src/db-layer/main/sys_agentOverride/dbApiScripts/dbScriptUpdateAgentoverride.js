...
// This script is called from the UpdateAgentOverrideManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.sys_agentOverride;
  return {
    old_sys_agentOverride: oldData,
    sys_agentOverride: newData,
...
const dbScriptUpdateAgentoverride = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptUpdateAgentoverride] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptUpdateAgentoverride] whereClause:",
      require("util").inspect(whereClause, { depth: 4 }),
    );

    const updateResult = await Sys_agentOverride.update(resolvedDataClause, {
    ...
    console.log(
      "[DbScript:dbScriptUpdateAgentoverride] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptUpdateAgentoverride] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await Sys_agentOverride.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptUpdateAgentoverride] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await Sys_agentOverride.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptUpdateAgentoverride] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateAgentoverride",
      {
...
module.exports = dbScriptUpdateAgentoverride;
