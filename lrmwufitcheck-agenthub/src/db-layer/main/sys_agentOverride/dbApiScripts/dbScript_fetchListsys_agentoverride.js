...
// This script is called from the _fetchListSys_agentOverrideManager

const { Sys_agentOverride, sequelize } = require("models");
...
const dbScript_fetchListsys_agentoverride = async (apiManager) => {
  ...
  try {
    ...
    let sys_agentOverrideList = await Sys_agentOverride.findAll(options);

    const dbData = { items: [] };
    apiManager.sys_agentOverrides = [];
    if (!sys_agentOverrideList || sys_agentOverrideList.length == 0)
      return dbData;

    dbData.items = Array.isArray(sys_agentOverrideList)
      ? sys_agentOverrideList
      : [sys_agentOverrideList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [
          sequelize.literal('COUNT(DISTINCT("sys_agentOverride".id))'),
          "_COUNT",
        ],
      ];
      let sys_agentOverride_count = await Sys_agentOverride.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      sys_agentOverride_count =
        sys_agentOverride_count && sys_agentOverride_count.length
          ? sys_agentOverride_count[0]._COUNT
          : 0;
      sys_agentOverride_count = isNaN(sys_agentOverride_count)
        ? sys_agentOverride_count
        : parseInt(sys_agentOverride_count);

      sys_agentOverrideList = {
        rows: sys_agentOverrideList,
        count: sys_agentOverride_count,
      };
      dbData.totalRowCount = sys_agentOverride_count;
    ...
    apiManager.sys_agentOverrides = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListsys_agentoverride",
      {
...
module.exports = dbScript_fetchListsys_agentoverride;
