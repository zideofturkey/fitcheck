...
const deleteSys_agentOverrideById = async (id, context = null) => {
  try {
    if (typeof id === "object") {
      id = id.id;
    }
    ...
    const existingDoc = await Sys_agentOverride.findByPk(id);
    if (!existingDoc) {
...
module.exports = deleteSys_agentOverrideById;
