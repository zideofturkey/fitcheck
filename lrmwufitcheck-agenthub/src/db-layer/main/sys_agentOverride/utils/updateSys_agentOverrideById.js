...
const updateSys_agentOverrideById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Sys_agentOverride.findOne({ where: { id } });
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await Sys_agentOverride.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateSys_agentOverrideById;
