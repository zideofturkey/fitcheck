...
async function checkBulkUniqueIndices(recordsToCreate) {
  ...
  // Check index: agentName-index (throwError)
  if (remaining.length > 0) {
    const getKey_agentName_index = (r) => String(r.agentName ?? null);

    const orConditions_agentName_index = remaining.map((data) => ({
      agentName: data.agentName ?? null,
    }));

    const conflicts_agentName_index = await Sys_agentOverride.findAll({
      where: { [Op.or]: orConditions_agentName_index },
    });

    if (conflicts_agentName_index.length > 0) {
      const conflictMap_agentName_index = new Map();
      conflicts_agentName_index.forEach((dbDoc) => {
        conflictMap_agentName_index.set(getKey_agentName_index(dbDoc), dbDoc);
      });

      // throwError: abort entire bulk operation if any record conflicts
      const matched_agentName_index = remaining.filter((d) =>
        conflictMap_agentName_index.has(getKey_agentName_index(d)),
      );
      if (matched_agentName_index.length > 0) {
...
const createBulkSys_agentOverride = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await Sys_agentOverride.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await Sys_agentOverride.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkSys_agentOverride",
      err,
    );
  }
};

module.exports = createBulkSys_agentOverride;
