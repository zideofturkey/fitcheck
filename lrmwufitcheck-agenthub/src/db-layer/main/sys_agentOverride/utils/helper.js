...
const objectName = "sys_agentOverride";
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for sys_agentOverride — no-op
};
 ...
 * on miss" pattern, use getCachedSys_agentOverrideById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for sys_agentOverride
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new Sys_agentOverrideQueryCacheInvalidator();
