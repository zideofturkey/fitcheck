...
function collectAllTools(headers) {
  ...
  const {
    sys_agentOverrideMcpRouter,
    sys_agentExecutionMcpRouter,
    sys_toolCatalogMcpRouter,
    sys_agentConversationMcpRouter,
    getSessionRouter,
  } = mcpExports;
  ...
  (sys_agentOverrideMcpRouter || []).forEach(addTool);
  (sys_agentExecutionMcpRouter || []).forEach(addTool);
  (sys_toolCatalogMcpRouter || []).forEach(addTool);
  (sys_agentConversationMcpRouter || []).forEach(addTool);
