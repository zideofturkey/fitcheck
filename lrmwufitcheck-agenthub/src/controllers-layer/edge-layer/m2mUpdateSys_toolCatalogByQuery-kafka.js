...
const { m2mUpdateSys_toolCatalogByQuery } = require("edgeFunctions");

const m2mUpdateSys_toolCatalogByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_toolCatalogByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_toolCatalogByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_toolCatalogByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_toolCatalogByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_toolCatalogByQueryHandler;
