...
const { m2mUpdateSys_agentOverrideByIdList } = require("edgeFunctions");

const m2mUpdateSys_agentOverrideByIdListHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentOverrideByIdListInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentOverrideByIdListRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentOverrideByIdList(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentOverrideByIdList: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentOverrideByIdListHandler;
