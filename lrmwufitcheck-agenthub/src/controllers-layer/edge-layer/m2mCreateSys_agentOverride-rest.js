...
const { m2mCreateSys_agentOverride } = require("edgeFunctions");
const createServiceController = require("restLayer/create-service-controller");

const m2mCreateSys_agentOverrideRestController = async (req, res, next) => {
  try {
    req.loginRequired = false;
    const statusCode = 201;

    const restController = createServiceController(
      "m2mCreateSys_agentOverride",
      "m2mCreateSys_agentOverride",
      req,
      res,
    );
    ...
    if (req.M2MToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mCreateSys_agentOverrideInvalidM2MToken",
    ...
    const result = await m2mCreateSys_agentOverride(req);
    ...
    console.error(
      "Error running routeService for m2mCreateSys_agentOverride: ",
      err,
    );
    return next(err);
  }
...
module.exports = m2mCreateSys_agentOverrideRestController;
