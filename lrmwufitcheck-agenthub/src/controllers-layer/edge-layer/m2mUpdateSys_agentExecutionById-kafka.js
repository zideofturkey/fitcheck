...
const { m2mUpdateSys_agentExecutionById } = require("edgeFunctions");

const m2mUpdateSys_agentExecutionByIdHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentExecutionByIdInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentExecutionByIdRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentExecutionById(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentExecutionById: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentExecutionByIdHandler;
