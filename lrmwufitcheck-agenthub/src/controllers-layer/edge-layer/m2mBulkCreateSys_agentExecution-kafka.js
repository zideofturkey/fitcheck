...
const { m2mBulkCreateSys_agentExecution } = require("edgeFunctions");

const m2mBulkCreateSys_agentExecutionHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mBulkCreateSys_agentExecutionInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mBulkCreateSys_agentExecutionRequiresM2MToken",
    ...
    const result = await m2mBulkCreateSys_agentExecution(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mBulkCreateSys_agentExecution: ",
      err,
    );
    // log error
  }
...
module.exports = m2mBulkCreateSys_agentExecutionHandler;
