...
const { m2mUpdateSys_agentOverrideByQuery } = require("edgeFunctions");

const m2mUpdateSys_agentOverrideByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentOverrideByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentOverrideByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentOverrideByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentOverrideByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentOverrideByQueryHandler;
