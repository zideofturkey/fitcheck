...
const { m2mUpdateSys_agentExecutionByQuery } = require("edgeFunctions");

const m2mUpdateSys_agentExecutionByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentExecutionByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentExecutionByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentExecutionByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentExecutionByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentExecutionByQueryHandler;
