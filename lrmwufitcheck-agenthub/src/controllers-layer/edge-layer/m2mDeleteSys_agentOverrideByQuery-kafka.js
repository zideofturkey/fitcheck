...
const { m2mDeleteSys_agentOverrideByQuery } = require("edgeFunctions");

const m2mDeleteSys_agentOverrideByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mDeleteSys_agentOverrideByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mDeleteSys_agentOverrideByQueryRequiresM2MToken",
    ...
    const result = await m2mDeleteSys_agentOverrideByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mDeleteSys_agentOverrideByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mDeleteSys_agentOverrideByQueryHandler;
