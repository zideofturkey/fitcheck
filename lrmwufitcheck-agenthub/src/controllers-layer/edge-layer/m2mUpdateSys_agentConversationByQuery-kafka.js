...
const { m2mUpdateSys_agentConversationByQuery } = require("edgeFunctions");

const m2mUpdateSys_agentConversationByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentConversationByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentConversationByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentConversationByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentConversationByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentConversationByQueryHandler;
