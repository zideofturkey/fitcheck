...
const { m2mUpdateSys_agentConversationById } = require("edgeFunctions");

const m2mUpdateSys_agentConversationByIdHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateSys_agentConversationByIdInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateSys_agentConversationByIdRequiresM2MToken",
    ...
    const result = await m2mUpdateSys_agentConversationById(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateSys_agentConversationById: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateSys_agentConversationByIdHandler;
