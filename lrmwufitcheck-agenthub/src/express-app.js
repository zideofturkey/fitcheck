...
const {
  ...
  sys_agentOverrideRouter,
  sys_agentExecutionRouter,
  sys_toolCatalogRouter,
  sys_agentConversationRouter,
} = require("restLayer");

app.use("", sys_agentOverrideRouter);
app.use("", sys_agentExecutionRouter);
app.use("", sys_toolCatalogRouter);
app.use("", sys_agentConversationRouter);

const agentRouter = require("agentLayer/agent-router");
const { initializeAgents } = require("agentLayer");
let _agentLib = {};
try {
  _agentLib = require("libFunctions");
} catch (_) {}
const agentRuntimes = initializeAgents({
  dbLayer: require("dbLayer"),
  lib: _agentLib,
});
app.agentRuntimes = agentRuntimes;
app.use("", agentRouter);

const { initToolCatalog } = require("hubLayer/tool-catalog-manager");
const agentHubRouter = require("hubLayer/agent-hub-router");
// Initialize tool catalog (discovers tools from other services)
initToolCatalog().catch((err) =>
  console.error("[AgentHub] Tool catalog init error:", err.message),
);
app.use("", agentHubRouter);
...
app.get("/getPostmanCollection", (req, res) => {
  const filePath = path.join(
    __dirname,
    "fitcheck-agentHub-postman-collection.json",
  );
  console.log("Downloading Postman Collection from:", filePath);
  res.setHeader("Content-Type", "application/json");
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="fitcheck-agentHub-postman-collection.json"',
  );
