...
const { dbScriptGetAgentoverride } = require("dbLayer");
...
class GetAgentOverrideManager extends Sys_agentOverrideManager {
  ...
  async getRouteQuery() {
    return runMScript(() => ({ id: this.sys_agentOverrideId }), {
      path: "services[5].businessLogic[0].whereClause.fullWhereClause",
    });
  ...
  async executeMainOperation() {
    return await dbScriptGetAgentoverride(this);
  ...
  async raiseEvent() {
    AgentoverrideRetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetAgentOverrideManager;
