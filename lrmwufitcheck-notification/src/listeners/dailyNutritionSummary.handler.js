...
module.exports = {
  topic: "fitcheck-mealtracker-service-nutritiondayclosed",
  handle: async (event) => {
    try {
      const mappedData = {
        types: ["email"],
        isStored: true,
        template: "dailyNutritionSummaryTemplate",
        metadata: {
          ...event,
          actionDeepLink: "",
          actionText: "",
      ...
      if (dataViewId == null) {
        console.warn(
          `fitcheck-mealtracker-service-nutritiondayclosed: no id resolvable in payload — skipping notification`,
          { keys: Object.keys(event || {}) },
        );
        return;
      }
      const dataSource = await getDocument(
        "lrmwufitcheck_DailyNutritionSummaryNotificationView",
        dataViewId,
      );
      if (!dataSource || !dataSource._source) {
        ...
        // halt the partition).
        console.warn(
          `fitcheck-mealtracker-service-nutritiondayclosed: ES record not found for id=${dataViewId} in index "lrmwufitcheck_DailyNutritionSummaryNotificationView" — skipping notification`,
        );
        return;
      }
      ...
      //**errorLog
      console.error("fitcheck-mealtracker-service-nutritiondayclosed ", error);
    }
  },
};
