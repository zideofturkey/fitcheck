...
describe("m2mDeleteAiSessionByQueryRestController", function () {
  let functionNameStub;
  let m2mDeleteAiSessionByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteAiSessionByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteAiSessionByQuery-rest",
      {
        edgeFunctions: { m2mDeleteAiSessionByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteAiSessionByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteAiSessionByQueryRestController = (req, res, next) => {
      next(error);
    };
