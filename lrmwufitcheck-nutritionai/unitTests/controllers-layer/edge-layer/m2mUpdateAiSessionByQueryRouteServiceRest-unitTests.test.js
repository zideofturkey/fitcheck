...
describe("m2mUpdateAiSessionByQueryRestController", function () {
  let functionNameStub;
  let m2mUpdateAiSessionByQueryRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mUpdateAiSessionByQueryRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mUpdateAiSessionByQuery-rest",
      {
        edgeFunctions: { m2mUpdateAiSessionByQuery: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mUpdateAiSessionByQueryRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mUpdateAiSessionByQueryRestController = (req, res, next) => {
      next(error);
    };
