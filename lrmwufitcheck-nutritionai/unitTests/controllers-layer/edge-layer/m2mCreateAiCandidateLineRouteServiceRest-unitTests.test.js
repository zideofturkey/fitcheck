...
describe("m2mCreateAiCandidateLineRestController", function () {
  let functionNameStub;
  let m2mCreateAiCandidateLineRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateAiCandidateLineRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateAiCandidateLine-rest",
      {
        edgeFunctions: { m2mCreateAiCandidateLine: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateAiCandidateLineRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateAiCandidateLineRestController = (req, res, next) => {
      next(error);
    };
