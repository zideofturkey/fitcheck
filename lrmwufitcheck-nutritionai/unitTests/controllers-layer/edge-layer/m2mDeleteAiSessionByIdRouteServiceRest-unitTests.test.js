...
describe("m2mDeleteAiSessionByIdRestController", function () {
  let functionNameStub;
  let m2mDeleteAiSessionByIdRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mDeleteAiSessionByIdRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mDeleteAiSessionById-rest",
      {
        edgeFunctions: { m2mDeleteAiSessionById: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mDeleteAiSessionByIdRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mDeleteAiSessionByIdRestController = (req, res, next) => {
      next(error);
    };
