...
describe("m2mBulkCreateAiSessionRestController", function () {
  let functionNameStub;
  let m2mBulkCreateAiSessionRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mBulkCreateAiSessionRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mBulkCreateAiSession-rest",
      {
        edgeFunctions: { m2mBulkCreateAiSession: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mBulkCreateAiSessionRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mBulkCreateAiSessionRestController = (req, res, next) => {
      next(error);
    };
