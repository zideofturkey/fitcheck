...
describe("m2mCreateAiSessionRestController", function () {
  let functionNameStub;
  let m2mCreateAiSessionRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateAiSessionRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateAiSession-rest",
      {
        edgeFunctions: { m2mCreateAiSession: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateAiSessionRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateAiSessionRestController = (req, res, next) => {
      next(error);
    };
