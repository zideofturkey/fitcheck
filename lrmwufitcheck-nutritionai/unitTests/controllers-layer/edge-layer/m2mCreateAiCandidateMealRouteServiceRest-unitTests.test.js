...
describe("m2mCreateAiCandidateMealRestController", function () {
  let functionNameStub;
  let m2mCreateAiCandidateMealRestController;

  beforeEach(() => {
    // Always return a basic object so controller can respond
    functionNameStub = sinon.stub().resolves({ content: "ok" });

    m2mCreateAiCandidateMealRestController = proxyquire(
      "../../../src/controllers-layer/edge-layer/m2mCreateAiCandidateMeal-rest",
      {
        edgeFunctions: { m2mCreateAiCandidateMeal: functionNameStub },
      },
    );
  });
  ...
  it("should return a valid response when login is not required", async function () {
    ...
    await m2mCreateAiCandidateMealRestController(req, res, next);
  ...
  it("should handle errors and call next() on failure", async function () {
    ...
    m2mCreateAiCandidateMealRestController = (req, res, next) => {
      next(error);
    };
