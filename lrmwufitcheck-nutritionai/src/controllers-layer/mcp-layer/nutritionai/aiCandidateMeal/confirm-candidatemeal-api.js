...
class ConfirmCandidateMealMcpController extends NutritionAiServiceMcpController {
  constructor(params) {
    super("confirmCandidateMeal", "confirmcandidatemeal", params);
    this.dataName = "aiCandidateMeal";
    this.crudType = "update";
  }

  createApiManager() {
    return new ConfirmCandidateMealManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      proposedMealDate: z
        ...
        .datetime()
        .optional()
        .describe("User may override the proposed date"),

      proposedMealTime: z
        .string()
        .max(255)
        .optional()
        .describe("User may override the proposed time"),

      proposedSlotName: z
        .string()
        .max(255)
        .optional()
        .describe("User may override the meal slot"),

      lineAdjustments: z.array(
        z
          .object({})
          .optional()
          .describe("Array of per-line gram/saveAsFood overrides"),
      ),
    };
  }
}
...
module.exports = (headers) => {
  ...
  return {
    ...
    description:
      ...
      requiredRolesMarker,
    requiredRoles,
    parameters: ConfirmCandidateMealMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ConfirmCandidateMealMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ConfirmCandidateMealMcpController.getOutputSchema().parse(result);
