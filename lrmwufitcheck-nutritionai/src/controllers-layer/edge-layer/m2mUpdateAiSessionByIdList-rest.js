...
const { m2mUpdateAiSessionByIdList } = require("edgeFunctions");
const createServiceController = require("restLayer/create-service-controller");

const m2mUpdateAiSessionByIdListRestController = async (req, res, next) => {
  try {
    req.loginRequired = false;
    const statusCode = 200;

    const restController = createServiceController(
      "m2mUpdateAiSessionByIdList",
      "m2mUpdateAiSessionByIdList",
      req,
      res,
    );
    ...
    if (req.M2MToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateAiSessionByIdListInvalidM2MToken",
    ...
    const result = await m2mUpdateAiSessionByIdList(req);
    ...
    console.error(
      "Error running routeService for m2mUpdateAiSessionByIdList: ",
      err,
    );
    return next(err);
  }
...
module.exports = m2mUpdateAiSessionByIdListRestController;
