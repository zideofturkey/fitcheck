...
const { m2mUpdateAiCandidateMealByQuery } = require("edgeFunctions");

const m2mUpdateAiCandidateMealByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mUpdateAiCandidateMealByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mUpdateAiCandidateMealByQueryRequiresM2MToken",
    ...
    const result = await m2mUpdateAiCandidateMealByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mUpdateAiCandidateMealByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mUpdateAiCandidateMealByQueryHandler;
