...
const { m2mDeleteAiCandidateMealByQuery } = require("edgeFunctions");

const m2mDeleteAiCandidateMealByQueryHandler = async (
  topic,
  session,
  message,
  callbackData,
  ...
  try {
    ...
    if (m2mToken) {
      ...
      if (!m2mPayload) {
        throw new NotAuthenticatedError(
          "m2mDeleteAiCandidateMealByQueryInvalidM2MToken",
      ...
      throw new NotAuthenticatedError(
        "m2mDeleteAiCandidateMealByQueryRequiresM2MToken",
    ...
    const result = await m2mDeleteAiCandidateMealByQuery(message);
    // log result
  } catch (err) {
    console.error(
      "Error running routeService for m2mDeleteAiCandidateMealByQuery: ",
      err,
    );
    // log error
  }
...
module.exports = m2mDeleteAiCandidateMealByQueryHandler;
