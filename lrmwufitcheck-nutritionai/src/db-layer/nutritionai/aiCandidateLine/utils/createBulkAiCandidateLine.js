...
const createBulkAiCandidateLine = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await AiCandidateLine.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await AiCandidateLine.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkAiCandidateLine",
      err,
    );
  }
};

module.exports = createBulkAiCandidateLine;
