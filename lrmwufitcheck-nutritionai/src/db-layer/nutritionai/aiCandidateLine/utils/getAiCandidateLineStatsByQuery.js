...
/**
 * @deprecated Use getAiCandidateLineStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getAiCandidateLineStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AiCandidateLine.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            AiCandidateLine.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            AiCandidateLine.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            AiCandidateLine.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            AiCandidateLine.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await AiCandidateLine.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingAiCandidateLineStatsByQuery",
      err,
    );
  }
};

module.exports = getAiCandidateLineStatsByQuery;
