...
/**
 * @deprecated Use updateAiCandidateLineByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateAiCandidateLineByQuery = async (
  dataClause,
  query,
  context = null,
) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await AiCandidateLine.findAll({
        where: whereClause,
      });
      ...
      [, rows] = await AiCandidateLine.update(dataClause, options);
    }
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiCandidateLineByQuery",
      err,
    );
  }
};

module.exports = updateAiCandidateLineByQuery;
