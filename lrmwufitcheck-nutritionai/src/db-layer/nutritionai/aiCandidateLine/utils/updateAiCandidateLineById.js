...
const updateAiCandidateLineById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AiCandidateLine.findOne({ where: { id } });
    ...
    const updateResult = await AiCandidateLine.update(
      resolvedDataClause,
      options,
    );
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await AiCandidateLine.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateAiCandidateLineById;
