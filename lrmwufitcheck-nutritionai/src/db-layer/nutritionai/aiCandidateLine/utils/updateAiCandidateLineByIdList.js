...
const updateAiCandidateLineByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const aiCandidateLineIdList = rows.map((item) => item.id);
    return aiCandidateLineIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiCandidateLineByIdList",
      err,
    );
  }
};

module.exports = updateAiCandidateLineByIdList;
