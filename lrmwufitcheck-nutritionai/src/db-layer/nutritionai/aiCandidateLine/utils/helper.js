...
const objectName = "aiCandidateLine";
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for aiCandidateLine — no-op
};
 ...
 * on miss" pattern, use getCachedAiCandidateLineById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for aiCandidateLine
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new AiCandidateLineQueryCacheInvalidator();
