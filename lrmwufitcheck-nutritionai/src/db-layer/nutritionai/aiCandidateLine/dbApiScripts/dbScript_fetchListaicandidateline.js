...
// This script is called from the _fetchListAiCandidateLineManager

const { AiCandidateLine, sequelize } = require("models");
...
const dbScript_fetchListaicandidateline = async (apiManager) => {
  ...
  try {
    ...
    let aiCandidateLineList = await AiCandidateLine.findAll(options);

    const dbData = { items: [] };
    apiManager.aiCandidateLines = [];
    if (!aiCandidateLineList || aiCandidateLineList.length == 0) return dbData;

    dbData.items = Array.isArray(aiCandidateLineList)
      ? aiCandidateLineList
      : [aiCandidateLineList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("aiCandidateLine".id))'), "_COUNT"],
      ];
      let aiCandidateLine_count = await AiCandidateLine.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      aiCandidateLine_count =
        aiCandidateLine_count && aiCandidateLine_count.length
          ? aiCandidateLine_count[0]._COUNT
          : 0;
      aiCandidateLine_count = isNaN(aiCandidateLine_count)
        ? aiCandidateLine_count
        : parseInt(aiCandidateLine_count);

      aiCandidateLineList = {
        rows: aiCandidateLineList,
        count: aiCandidateLine_count,
      };
      dbData.totalRowCount = aiCandidateLine_count;
    ...
    apiManager.aiCandidateLines = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListaicandidateline",
      {
...
module.exports = dbScript_fetchListaicandidateline;
