...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for aiSession — no-op
};
 ...
 * on miss" pattern, use getCachedAiSessionById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for aiSession
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new AiSessionQueryCacheInvalidator();
