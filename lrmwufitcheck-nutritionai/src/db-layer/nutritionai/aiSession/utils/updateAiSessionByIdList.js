...
const updateAiSessionByIdList = async (idList, dataClause, context = null) => {
  try {
    let rows = [];
    ...
    const aiSessionIdList = rows.map((item) => item.id);
    return aiSessionIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiSessionByIdList",
      err,
    );
  }
};

module.exports = updateAiSessionByIdList;
