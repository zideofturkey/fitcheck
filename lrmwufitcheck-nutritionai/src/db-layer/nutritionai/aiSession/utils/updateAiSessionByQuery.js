...
/**
 * @deprecated Use updateAiSessionByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateAiSessionByQuery = async (dataClause, query, context = null) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await AiSession.findAll({ where: whereClause });
      for (const item of existingRows) {
        const resolvedDataClause = resolveArrayMutationsInClause(
          dataClause,
          item.getData(),
        );
        const updateResult = await AiSession.update(resolvedDataClause, {
          where: { id: item.id },
          returning: true,
        });
      ...
      [, rows] = await AiSession.update(dataClause, options);
    }
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiSessionByQuery",
      err,
    );
  }
};

module.exports = updateAiSessionByQuery;
