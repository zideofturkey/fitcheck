...
const createBulkAiSession = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await AiSession.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await AiSession.bulkCreate(finalRecordsToCreate, {
        returning: true,
        validate: true,
      });
    ...
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenCreatingBulkAiSession", err);
  }
};

module.exports = createBulkAiSession;
