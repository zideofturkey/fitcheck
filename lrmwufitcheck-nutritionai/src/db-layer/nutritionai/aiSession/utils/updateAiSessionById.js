...
const updateAiSessionById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AiSession.findOne({ where: { id } });
    ...
    const updateResult = await AiSession.update(resolvedDataClause, options);
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await AiSession.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateAiSessionById;
