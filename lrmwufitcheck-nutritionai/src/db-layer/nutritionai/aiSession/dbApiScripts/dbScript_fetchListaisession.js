...
// This script is called from the _fetchListAiSessionManager

const { AiSession, sequelize } = require("models");
...
const dbScript_fetchListaisession = async (apiManager) => {
  ...
  try {
    ...
    let aiSessionList = await AiSession.findAll(options);

    const dbData = { items: [] };
    apiManager.aiSessions = [];
    if (!aiSessionList || aiSessionList.length == 0) return dbData;

    dbData.items = Array.isArray(aiSessionList)
      ? aiSessionList
      : [aiSessionList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("aiSession".id))'), "_COUNT"],
      ];
      let aiSession_count = await AiSession.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      aiSession_count =
        aiSession_count && aiSession_count.length
          ? aiSession_count[0]._COUNT
          : 0;
      aiSession_count = isNaN(aiSession_count)
        ? aiSession_count
        : parseInt(aiSession_count);

      aiSessionList = {
        rows: aiSessionList,
        count: aiSession_count,
      };
      dbData.totalRowCount = aiSession_count;
    ...
    apiManager.aiSessions = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListaisession",
      {
...
module.exports = dbScript_fetchListaisession;
