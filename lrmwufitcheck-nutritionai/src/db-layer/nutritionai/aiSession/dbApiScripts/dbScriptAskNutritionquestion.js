...
// This script is called from the AskNutritionQuestionManager

const { AiSession } = require("models");

const { AiSessionQueryCacheInvalidator } = require("./query-cache-classes");
...
async function updateIfExists(dataClause) {
  const dbDoc = await AiSession.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptAskNutritionquestion = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await AiSession.create(dataClause);
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    apiManager.aiSession = dbData;
    ...
    const queryCacheInvalidator = new AiSessionQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptAskNutritionquestion",
      {
        whereClause: null,
...
module.exports = dbScriptAskNutritionquestion;
