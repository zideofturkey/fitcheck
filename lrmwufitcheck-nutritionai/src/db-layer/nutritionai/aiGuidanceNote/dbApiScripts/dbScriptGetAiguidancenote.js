...
// This script is called from the GetAiGuidanceNoteManager
...
const dbScriptGetAiguidancenote = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await AiGuidanceNote.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.aiGuidanceNote = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetAiguidancenote",
      {
...
module.exports = dbScriptGetAiguidancenote;
