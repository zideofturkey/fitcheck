...
/**
 * @deprecated Use getAiGuidanceNoteStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getAiGuidanceNoteStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AiGuidanceNote.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            AiGuidanceNote.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            AiGuidanceNote.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            AiGuidanceNote.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            AiGuidanceNote.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await AiGuidanceNote.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingAiGuidanceNoteStatsByQuery",
      err,
    );
  }
};

module.exports = getAiGuidanceNoteStatsByQuery;
