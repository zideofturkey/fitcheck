...
const createBulkAiGuidanceNote = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await AiGuidanceNote.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await AiGuidanceNote.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkAiGuidanceNote",
      err,
    );
  }
};

module.exports = createBulkAiGuidanceNote;
