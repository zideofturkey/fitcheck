...
/**
 * @deprecated Use updateAiGuidanceNoteByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateAiGuidanceNoteByQuery = async (
  dataClause,
  query,
  context = null,
) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await AiGuidanceNote.findAll({ where: whereClause });
      for (const item of existingRows) {
        const resolvedDataClause = resolveArrayMutationsInClause(
          dataClause,
          item.getData(),
        );
        const updateResult = await AiGuidanceNote.update(resolvedDataClause, {
          where: { id: item.id },
          returning: true,
        });
      ...
      [, rows] = await AiGuidanceNote.update(dataClause, options);
    }
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiGuidanceNoteByQuery",
      err,
    );
  }
};

module.exports = updateAiGuidanceNoteByQuery;
