...
const updateAiGuidanceNoteById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AiGuidanceNote.findOne({ where: { id } });
    ...
    const updateResult = await AiGuidanceNote.update(
      resolvedDataClause,
      options,
    );
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await AiGuidanceNote.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateAiGuidanceNoteById;
