...
const objectName = "aiGuidanceNote";
...
const raiseDbEventUpdate = async (
  ...
  const eventData = {
    old_aiGuidanceNote: oldData,
    aiGuidanceNote: newData,
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for aiGuidanceNote — no-op
};
 ...
 * on miss" pattern, use getCachedAiGuidanceNoteById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for aiGuidanceNote
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new AiGuidanceNoteQueryCacheInvalidator();
