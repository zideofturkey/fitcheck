...
const updateAiGuidanceNoteByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const aiGuidanceNoteIdList = rows.map((item) => item.id);
    return aiGuidanceNoteIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiGuidanceNoteByIdList",
      err,
    );
  }
};

module.exports = updateAiGuidanceNoteByIdList;
