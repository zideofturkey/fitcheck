...
// This script is called from the _fetchListAiCandidateMealManager

const { AiCandidateMeal, sequelize } = require("models");
...
const dbScript_fetchListaicandidatemeal = async (apiManager) => {
  ...
  try {
    ...
    let aiCandidateMealList = await AiCandidateMeal.findAll(options);

    const dbData = { items: [] };
    apiManager.aiCandidateMeals = [];
    if (!aiCandidateMealList || aiCandidateMealList.length == 0) return dbData;

    dbData.items = Array.isArray(aiCandidateMealList)
      ? aiCandidateMealList
      : [aiCandidateMealList];
    ...
    if (apiManager.pagination && apiManager.pagination.pageNumber) {
      let attributes = [
        [sequelize.literal('COUNT(DISTINCT("aiCandidateMeal".id))'), "_COUNT"],
      ];
      let aiCandidateMeal_count = await AiCandidateMeal.findAll({
        where: whereClause,
        raw: true,
        attributes,
      });
      aiCandidateMeal_count =
        aiCandidateMeal_count && aiCandidateMeal_count.length
          ? aiCandidateMeal_count[0]._COUNT
          : 0;
      aiCandidateMeal_count = isNaN(aiCandidateMeal_count)
        ? aiCandidateMeal_count
        : parseInt(aiCandidateMeal_count);

      aiCandidateMealList = {
        rows: aiCandidateMealList,
        count: aiCandidateMeal_count,
      };
      dbData.totalRowCount = aiCandidateMeal_count;
    ...
    apiManager.aiCandidateMeals = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScript_fetchListaicandidatemeal",
      {
...
module.exports = dbScript_fetchListaicandidatemeal;
