...
// This script is called from the RejectCandidateMealManager
...
const dbScriptRejectCandidatemeal = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptRejectCandidatemeal] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptRejectCandidatemeal] whereClause:",
    ...
    console.log(
      "[DbScript:dbScriptRejectCandidatemeal] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptRejectCandidatemeal] re-fetching after update (returning was empty but rowsCount > 0)",
      );
    ...
    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptRejectCandidatemeal] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await AiCandidateMeal.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptRejectCandidatemeal] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptRejectCandidatemeal",
      {
...
module.exports = dbScriptRejectCandidatemeal;
