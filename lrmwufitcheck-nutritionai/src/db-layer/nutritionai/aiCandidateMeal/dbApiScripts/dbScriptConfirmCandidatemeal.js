...
// This script is called from the ConfirmCandidateMealManager
...
function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.aiCandidateMeal;
  return {
    old_aiCandidateMeal: oldData,
    aiCandidateMeal: newData,
...
const dbScriptConfirmCandidatemeal = async (apiManager) => {
  ...
  try {
    ...
    console.log(
      "[DbScript:dbScriptConfirmCandidatemeal] dataClause after clear:",
      JSON.stringify(resolvedDataClause),
    );
    console.log(
      "[DbScript:dbScriptConfirmCandidatemeal] whereClause:",
      require("util").inspect(whereClause, { depth: 4 }),
    );

    const updateResult = await AiCandidateMeal.update(resolvedDataClause, {
    ...
    console.log(
      "[DbScript:dbScriptConfirmCandidatemeal] updateResult: rowsCount=",
      rowsCount,
      "hasReturningData=",
      !!dbDoc,
      "hasDbData=",
    ...
    if (!dbData && rowsCount > 0) {
      console.log(
        "[DbScript:dbScriptConfirmCandidatemeal] re-fetching after update (returning was empty but rowsCount > 0)",
      );
      dbDoc = await AiCandidateMeal.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    if (!dbData && rowsCount === 0 && oldDbData?.id) {
      console.log(
        "[DbScript:dbScriptConfirmCandidatemeal] update matched 0 rows — re-fetching by id:",
        oldDbData.id,
      );
      dbDoc = await AiCandidateMeal.findOne({ where: instanceWhereClause });
      dbData = dbDoc ? dbDoc.getData() : null;
    ...
    if (!dbData) {
      console.log(
        "[DbScript:dbScriptConfirmCandidatemeal] FAILED: no data after update. rowsCount=",
        rowsCount,
        "updateResult length=",
        updateResult.length,
        "updateResult[1] type=",
    ...
    apiManager.aiCandidateMeal = dbData;
    ...
    const queryCacheInvalidator = new AiCandidateMealQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptConfirmCandidatemeal",
      {
...
module.exports = dbScriptConfirmCandidatemeal;
