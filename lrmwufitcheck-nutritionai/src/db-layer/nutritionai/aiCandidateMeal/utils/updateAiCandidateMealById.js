...
const updateAiCandidateMealById = async (id, dataClause, context = null) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AiCandidateMeal.findOne({ where: { id } });
    ...
    const updateResult = await AiCandidateMeal.update(
      resolvedDataClause,
      options,
    );
    ...
    if (!dbDoc && rowsCount > 0) {
      dbDoc = await AiCandidateMeal.findOne({ where: options.where });
    }
    if (!dbDoc) {
...
module.exports = updateAiCandidateMealById;
