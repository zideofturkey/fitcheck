...
const objectName = "aiCandidateMeal";
const serviceCodename = "lrmwufitcheck-nutritionai-service";
...
const _saveEntityToCache = async (data) => {
  // entity caching is disabled for aiCandidateMeal — no-op
};
 ...
 * on miss" pattern, use getCachedAiCandidateMealById instead.
...
const getEntityFromCache = async (id) => {
  // entity caching is disabled for aiCandidateMeal
  return null;
};
...
const invalidateQueryCache = async (newData, oldData = null) => {
  try {
    const invalidator = new AiCandidateMealQueryCacheInvalidator();
