...
/**
 * @deprecated Use getAiCandidateMealStatsByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const getAiCandidateMealStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AiCandidateMeal.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            AiCandidateMeal.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          ...
          promises.push(
            AiCandidateMeal.aggregate(statParts[1], "avg", {
          ...
          promises.push(
            AiCandidateMeal.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            AiCandidateMeal.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await AiCandidateMeal.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingAiCandidateMealStatsByQuery",
      err,
    );
  }
};

module.exports = getAiCandidateMealStatsByQuery;
