...
const createBulkAiCandidateMeal = async (dataList, context = null) => {
  try {
    ...
    if (idsToCheck.length > 0) {
      const existingRecords = await AiCandidateMeal.findAll({
        where: {
          id: {
            [Op.in]: idsToCheck,
          },
    ...
    if (finalRecordsToCreate.length > 0) {
      const bulkCreated = await AiCandidateMeal.bulkCreate(
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenCreatingBulkAiCandidateMeal",
      err,
    );
  }
};

module.exports = createBulkAiCandidateMeal;
