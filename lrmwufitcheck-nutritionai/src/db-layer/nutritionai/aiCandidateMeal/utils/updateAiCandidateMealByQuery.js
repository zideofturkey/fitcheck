...
/**
 * @deprecated Use updateAiCandidateMealByMQuery instead, which accepts database-agnostic MScript Query format.
 */
const updateAiCandidateMealByQuery = async (
  dataClause,
  query,
  context = null,
) => {
  try {
    ...
    if (hasArrayMutations(dataClause)) {
      const existingRows = await AiCandidateMeal.findAll({
        where: whereClause,
      });
      ...
      [, rows] = await AiCandidateMeal.update(dataClause, options);
    }
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiCandidateMealByQuery",
      err,
    );
  }
};

module.exports = updateAiCandidateMealByQuery;
