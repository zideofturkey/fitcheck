...
const updateAiCandidateMealByIdList = async (
  idList,
  dataClause,
  context = null,
) => {
  try {
    ...
    const aiCandidateMealIdList = rows.map((item) => item.id);
    return aiCandidateMealIdList;
  } catch (err) {
    //**errorLog
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAiCandidateMealByIdList",
      err,
    );
  }
};

module.exports = updateAiCandidateMealByIdList;
